...
from nro.engine.bids import parse_bids_entities, resolve_bids_metadata
...
def parse_acq_time(raw: str) -> float:
    item = str(raw).strip()
    match = re.match(r"^(\d{1,2}):(\d{1,2}):(\d{1,2})(?:\.(\d+))?$", item)
    if not match:
        raise ValueError(f"Unrecognized AcquisitionTime: {raw!r}")
    hh = int(match.group(1))
    mm = int(match.group(2))
    ss = int(match.group(3))
    frac = match.group(4) or "0"
    usec = int((frac + "000000")[:6])
    t = time(hour=hh, minute=mm, second=ss, microsecond=usec)
...
def read_nifti_shape_and_zooms(img: Path) -> tuple[tuple[int, int, int], tuple[float, float, float]]:
    try:
    ...
    if sizeof_hdr_le == 348:
        ...
        raise RuntimeError(f"Unrecognized NIfTI header size for {img}: {sizeof_hdr_le}/{sizeof_hdr_be}")
...
def same_voxel_sizes(a: tuple[float, float, float], b: tuple[float, float, float], *, tol: float = 1e-6) -> bool:
...
def load_rec(img: Path) -> ImageRec:
    ...
    tkind, tval = get_time_key(meta, img)
...
def pick_prev_sbref(sbrefs: Sequence[ImageRec], bold: ImageRec) -> ImageRec:
    ...
    if not cands:
        raise RuntimeError(f"No previous SBRef with PhaseEncodingDirection={bold.ped!r} found for {bold.img.name}")
...
def pick_nearest_opp_sbref(sbrefs: Sequence[ImageRec], sbref: ImageRec) -> ImageRec:
    ...
    if not cands_all:
        raise RuntimeError(f"No opposite-PE SBRef found for {sbref.img.name} (need PhaseEncodingDirection={opp!r}).")
    ...
    def ok_geom(s: ImageRec) -> bool:
        try:
            return nifti_shape3(s.img) == sbref_shape and same_voxel_sizes(voxel_sizes_3d(s.img), sbref_zooms)
        except Exception:
            return False
...
def pick_prev_fmap_pair(fmaps: Sequence[ImageRec], bold: ImageRec) -> FmapPair:
    ...
    if intended:
        ...
        prev = [f for f in fmaps if f.ped is not None and f.tkind == bold.tkind and f.tval < bold.tval]
    ...
    compatible_pairs = [(same, opposite) for same in same_dir for opposite in opp_dir if pair_is_compatible(same, opposite)]
...
def _match_entities(candidate: dict[str, str], target: dict[str, str], *, ignore: Sequence[str] = ()) -> bool:
    ignored = set(ignore)
...
def resolve_bold_from_run_stem(func_dir: Path, *, sub_id: str, ses_id: Optional[str], run_stem: str) -> tuple[Path, Optional[str]]:
    ...
    if len(fallback) > 1:
        ...
        if target_task:
            ...
            for path in fallback:
                ...
                if cand_task and (cand_task.startswith(target_task) or target_task.startswith(cand_task)):
...
def resolve_func_run_request(
    *,
    ...
    bold_path, stem_warning = resolve_bold_from_run_stem(func_dir, sub_id=sub_id, ses_id=ses_id, run_stem=run_stem)
    ...
    return ResolvedFuncRun(
        ...
        resolved_run_stem=nii_stem(bold.img).removesuffix("_bold"),
    )
