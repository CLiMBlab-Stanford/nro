...
def _exponential_loukas_weights(
    correlations: np.ndarray, temperature: float
) -> np.ndarray:
...
def _coarsening_targets(
    n_nodes: int, target: int, iterations: int
) -> tuple[int, ...]:
    ...
    candidates = (
        target * (1 << exponent)
        for exponent in range(iterations - 1, -1, -1)
    )
    steps: list[int] = []
    current = n_nodes
...
def build_module(
    ...
    *,
    ...
    if cfg.inputs.domain == "surface":
        ...
        mask = load_mask(surface_mask, vertex_counts)
        volume_space = None
    else:
    ...
    step_targets = _coarsening_targets(
        int(mask.sum()), target, cfg.coarsening.iterations
    )
    previous_labels_checkpoint: Path | None = None
    correlation_checkpoints: list[Path] = []
    label_checkpoints: list[Path] = []
    run_to_index = {
        run: index for index, run in enumerate(cfg.inputs.functional)
    }
    ...
    for pass_index, step_target in enumerate(step_targets, start=1):
        ...
        progress_label = (
            f"Streaming coarsening pass {pass_index}/{len(step_targets)}"
        )
        ...
        runner.add_step(
            Step.python(
                name=(
                    f"Microparcellation Pass {pass_index}: "
                    "Stream Edge Correlations"
                ),
        ...
        def coarsen_step(
            *,
            ...
            if correlations.shape != (len(current_edges),):
                raise RuntimeError(
                    f"Invalid edge-correlation checkpoint: {correlations_path}"
                )
    ...
    def write_connectivity() -> None:
        ...
        active_stage_files = tuple(
            cfg.inputs.functional[int(index)] for index in included_indices
        )
        ...
        if cfg.inputs.domain == "surface":
            parcel_axis = surface_parcel_axis(
                labels, vertex_counts, cfg.inputs.surface
            )
        else:
            assert active_volume_space is not None
        ...
        quality = {
            ...
            "null_baseline": {
                ...
                "fitted_minus_null_mean": float(
                    result.variance_preserved - null_scores.mean()
                ),
            },
        }
    ...
    def write_manifest() -> None:
        ...
        manifest = {
            ...
            "n_surface_vertices": (
                n_vertices if cfg.inputs.domain == "surface" else None
            ),
            "hemisphere_vertex_counts": list(vertex_counts),
            "n_active_nodes": int(mask.sum()),
            "n_active_vertices": (
                int(mask.sum()) if cfg.inputs.domain == "surface" else None
            ),
            "n_gray_matter_voxels": (
                n_vertices if cfg.inputs.domain == "volume" else None
            ),
            ...
            "source_volume_mask": (
                str(cfg.inputs.mask) if cfg.inputs.domain == "volume" else None
            ),
            "volume_mask_resampled": (
                volume_space.mask_resampled if volume_space is not None else None
            ),
            "volume_connectivity": (
                cfg.inputs.volume_connectivity
                if cfg.inputs.domain == "volume"
                else None
            ),
            "functional_runs": {
            ...
            "outputs": {
                name: manifest_value(path) for name, path in outputs.items()
            },
    ...
    def validate_manifest() -> tuple[bool, str]:
        ...
        if (
            ...
            and published.get("configuration_fingerprint") != fingerprint
        ):
