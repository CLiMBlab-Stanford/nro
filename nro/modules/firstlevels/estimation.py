...
def declared_contrasts(node: dict, names: list[str]) -> list[dict]:
    ...
    if dummy:
        ...
        for name in candidates:
            if any(fnmatch.fnmatchcase(name, "intercept" if p == 1 else p) for p in patterns):
                contrasts.append(
                    {"Name": name, "ConditionList": [name], "Weights": [1], "Test": dummy["Test"]}
                )
    ...
    if any(not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*", c["Name"]) for c in contrasts):
        raise ValueError(
            "Contrast names must contain only letters, digits, dots, underscores and hyphens"
        )
    return contrasts


def run_records(
    node: dict, names: list[str], design, run_key: str, entities: dict
) -> tuple[list, list]:
        ...
        records.append(
            {
                ...
                "test": contrast["Test"],
                "entities": entities,
                "internal": contrast["Name"]
                in node["Model"].get("Software", {}).get("nro", {}).get("internal_contrasts", []),
...
def meta_records(
    node: dict, records: list[dict], edge: dict, *, weighting: str
) -> tuple[list, list]:
    ...
    for grouping, selected in groups.items():
            ...
            for pattern in node["Model"]["X"]:
                ...
                for name in names:
                    if fnmatch.fnmatchcase(name, pattern):
                        by_name[name] = {
                            "kind": weighting,
                            "inputs": [r["recipe"] for r in selected if r["name"] == name],
                        }
        ...
        for contrast in declared_contrasts(node, declared):
            ...
            output_name = (
                str(entities.get("contrast", contrast["Name"]))
                if contrast["Name"] == "intercept"
                else contrast["Name"]
            )
            if absent:
                omitted.append(
                    {
                        "contrast": output_name,
                        "entities": entities,
...
def evaluate_maps(recipe: dict, sources: dict, *, block_size: int) -> dict[str, np.ndarray]:
    ...
    result = {
        name: np.empty(n_locations, dtype=np.float32) for name in ("effect", "variance", "t", "dof")
    }
