...
def contrast_weights(contrast: dict) -> np.ndarray:
    ...
    try:
        ...
        raise ValueError(
            "Contrast weights must be a vector of finite numbers or fractions"
        ) from error
    if (
        len(values) != len(contrast["ConditionList"])
        or not len(values)
...
def validate_model(document: dict, *, task: str) -> dict:
    ...
    _keys(
        result.get("Input"),
        {"task", "subject", "session", "run", "acquisition", "direction"},
        "Input",
    )
    ...
    for node in nodes:
        _keys(
            node,
            {"Name", "Level", "GroupBy", "Transformations", "Model", "Contrasts", "DummyContrasts"},
            "node",
        )
        ...
        if level not in {"Run", "Session", "Subject"}:
            raise ValueError(
                "Only Run, Session and Subject levels are supported; no Dataset analyses"
            )
        ...
        if (
            ...
            or not set(grouping) <= allowed_groups
            or "subject" not in grouping
        ):
        ...
        if model.get("Type") != expected:
            raise ValueError(
                f"{level} requires Model.Type={expected}; summary-level GLMs are forbidden"
            )
        x = model.get("X")
        ...
        for contrast in node.get("Contrasts", []):
            ...
            if (
                ...
                or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*", cname)
                or cname in contrast_names
            ):
            ...
            if not isinstance(conditions, list) or any(
                v != 1 and not isinstance(v, str) for v in conditions
            ):
        ...
        if dummy:
            ...
            if "Contrasts" in dummy and (
                not isinstance(dummy["Contrasts"], list)
                or any(v != 1 and not isinstance(v, str) for v in dummy["Contrasts"])
            ):
    ...
    edges = result.setdefault(
        "Edges", [{"Source": a, "Destination": b} for a, b in zip(names, names[1:])]
    )
    parents = {}
    for edge in edges:
        ...
        if {"Run": 0, "Session": 1, "Subject": 2}[levels[source]] >= {
            "Run": 0,
            "Session": 1,
            "Subject": 2,
        }[levels[dest]]:
...
def validate_run_groups(runs, node: dict, participant: str) -> None:
    ...
    for run in runs:
        ...
        if group in groups:
            raise ValueError(
                "Run GroupBy must uniquely identify each BOLD run; add session/run/direction as needed"
            )
        groups.add(group)


def expand_columns(
    patterns: list, available: list[str], *, factor_variables: tuple[str, ...] = ()
) -> list[str]:
...
def event_design(
    node: dict, events: pd.DataFrame, confounds: pd.DataFrame, tr: float
) -> pd.DataFrame:
    ...
    if (
        not np.isfinite(events[["onset", "duration"]].to_numpy(float)).all()
        or (events.duration < 0).any()
    ):
        raise ValueError("Events need finite onsets and nonnegative durations")
    sparse, factor_variables, convolutions = event_variables(
        events, node.get("Transformations", {})
    )
    hrf = node["Model"].get("HRF")
    if hrf:
        for pattern in hrf["Variables"]:
            if not any(fnmatch.fnmatchcase(name, pattern) for name in sparse) and not any(
                pattern.startswith(f"{name}.") for name in factor_variables
            ):
    ...
    columns = expand_columns(
        node["Model"]["X"], list(values), factor_variables=tuple(factor_variables)
    )
