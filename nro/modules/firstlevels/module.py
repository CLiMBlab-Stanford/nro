...
def _write_manifest(
    ...
    value = {
        **base,
        "complete": True,
        "public_outputs": sorted(set(str(p) for p in outputs)),
        "records": records,
        "omissions": omissions,
        "source_fits": sources,
...
def _publish_records(
    ...
    for record in records:
        if record.get("internal"):
            continue
        entities = {
            {"session": "ses", "direction": "dir", "acquisition": "acq"}.get(k, k): v
            for k, v in record["entities"].items()
        }
        extra = "".join(
            f"_{key}-{value}"
            for key, value in entities.items()
            if key not in {"subject", "task"}
...
def create_fit_step(
    *,
    ...
    def action() -> None:
                ...
                omissions=[
                    {
                        "run": run.stem,
                        "contrast": None,
                        "reason": "unidentifiable_temporal_model",
                        "detail": str(error),
                    }
                ],
            )
        ...
        entities.update(
            {
                {"ses": "session", "dir": "direction", "acq": "acquisition"}.get(k, k): v
                for k, v in run.entities.items()
                if k != "sub"
            }
        )
        key = run.stem
...
def create_meta_step(
    *, inputs: tuple[Path, ...], edge: dict, node: dict, prefix: Path, base: dict, config: dict
) -> Step:
    ...
    def action() -> None:
        ...
        for candidate in spatial[1:]:
            if any(
                candidate.get(key) != geometry.get(key) for key in ("domain", "shape", "counts")
            ):
                raise ValueError("Run geometries differ within one firstlevels target")
            if geometry["domain"] == "volume" and not np.allclose(
                candidate["affine"], geometry["affine"]
            ):
        ...
        with atomic_output_path(output) as staged:
            if surface is not None:
                ...
                values = gaussian_filter(
                    np.asarray(image.dataobj, dtype=np.float32), (*sigma, 0), mode="constant"
                )
                nib.save(nib.Nifti1Image(values, image.affine, image.header), staged)

    return Step.python(
        name=f"Smooth {source.name}",
...
def run_module(
    *,
            ...
            for index, source in enumerate(original):
                ...
                if source.name.endswith(".gii"):
                    if space == "fsnative":
                        ...
                        anatomy_manifest = (
                            anatomy / f"sub-{participant}_desc-preprocessAnat_manifest.json"
                        )
                        if execution_context is not None:
                        ...
                        surface = find_fsaverage_surface(
                            hemi="L" if index == 0 else "R",
                            surface="midthickness",
                            n_vertices=count_vertices,
                        )
                output = work_root / prefix / source.name
                runner.add_step(
                    create_smoothing_step(
                        source,
                        output,
                        smoothing=smoothing,
                        surface=surface,
    ...
    for node in model["Nodes"][1:]:
        ...
        step = create_meta_step(
            inputs=tuple(manifests[edge["Source"]]),
            edge=edge,
            node=node,
            prefix=node_prefix(root, prefix, node),
            base={**base, "node": node["Name"]},
            config=config,
        )
