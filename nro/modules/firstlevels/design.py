...
def build_design(
    node: dict, events: pd.DataFrame, confounds: pd.DataFrame, tr: float, config: dict
) -> RunDesign:
    ...
    components, metadata = nuisance_components(
        exact,
        nuisance,
        variance_target=config["nuisance_variance_explained"],
        minimum_rank=config["minimum_temporal_rank"],
        minimum_fraction=config["minimum_temporal_rank_fraction"],
    )
    ...
    metadata.update(
        {
            "Predictors": names,
            "NuisanceCandidates": nuisance_names,
            "FitColumns": [names[i] for i in selected]
            + [f"nuisancePC-{i + 1}" for i in range(components.shape[1])],
            "TotalFrames": len(full),
            "RetainedFrameIndices": np.flatnonzero(retained).tolist(),
            "TemporalMaskColumns": outlier_names,
            "ObservationDimension": int(retained.sum()),
            ...
            "StatsModelNode": node,
            "PredictorSources": {
                **{n: "events.tsv" for n in rules["event_columns"]},
