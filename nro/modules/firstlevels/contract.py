...
def validate_completion(path: Path, *, definition: dict | None = None) -> tuple[bool, str]:
    ...
    try:
        ...
        if value["node"] != "module" and value["records"]:
            ...
            fields = (
                {"counts"}
                if geometry["domain"] == "surface"
                else {"shape", "affine", "reference_path"}
            )
        ...
        for source in value["source_fits"].values():
            if source["dof"] <= 0 or set(source["arrays"]) != {
                "beta",
                "residual_variance",
                "groups",
                "covariance",
...
def validate_public_definition(manifest: dict, processing: dict) -> tuple[bool, str]:
    ...
    try:
        matches = expected is not None and scientific_model(
            manifest.get("task_model")
        ) == scientific_model(expected)
