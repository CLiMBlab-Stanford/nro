...
def write_design(prefix: Path, design) -> tuple[Path, ...]:
    ...
    atomic_write_text(
        numerical, pd.DataFrame(expanded, columns=names).to_csv(sep="\t", index=False)
    )
...
def design_plot(path: Path, matrix: np.ndarray, names: list[str], retained: np.ndarray) -> None:
    ...
    parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height + top + 40}">',
    ...
    for column, name in enumerate(names):
        ...
        parts.append(
            f'<text transform="translate({x:.2f},150) rotate(-65)" font-size="10">{escape(name)}</text>'
        )
    for row in range(len(values)):
        for column in range(values.shape[1]):
            ...
            if not retained[row]:
                color = "#888888"
            parts.append(
                f'<rect x="{left + column * dx:.2f}" y="{top + row * dy:.2f}" width="{dx + 0.1:.2f}" height="{dy + 0.1:.2f}" fill="{color}"/>'
            )
    parts.extend(
        [
            f'<text x="12" y="{top + height + 25}" font-size="11">Rows: original frames. Gray: censored. Columns scaled only for display.</text>',
            "</svg>",
        ]
    )
