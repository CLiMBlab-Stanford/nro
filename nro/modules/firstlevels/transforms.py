...
def validate_transform(instruction: dict) -> None:
    ...
    if not isinstance(instruction, dict) or instruction.get("Name") not in options:
        raise ValueError(
            "Supported transformations: Factor, Demean, Scale, Sum, Product, Select, Convolve"
        )
    ...
    if (
        ...
        or not inputs
        or any(not isinstance(v, str) or not v for v in inputs)
    ):
    ...
    if "Output" in instruction and (
        not isinstance(instruction["Output"], str) or not instruction["Output"]
    ):
    ...
    if name in {"Scale", "Demean"}:
        ...
        if "Groupby" in instruction and (
            not isinstance(instruction["Groupby"], list)
            or any(not isinstance(v, str) for v in instruction["Groupby"])
        ):
...
def event_variables(events: pd.DataFrame, transformations: dict) -> tuple[dict, list, dict]:
    ...
    """
    sparse = {
        name: events[name].to_numpy(float)
        for name in events
    ...
    for instruction in transformations.get("Instructions", []):
        ...
        if operation == "Factor":
            ...
            for column in patterns:
                ...
                for value in sorted(events[column].dropna().unique(), key=str):
                    if (
                        instruction.get("Constraint") == "drop_one"
                        and value == instruction["RefLevel"]
                    ):
                        continue
                    name = f"{column}.{value}"
                    if name in sparse:
        ...
        for pattern in patterns:
            ...
            if not matches and not any(pattern.startswith(f"{f}.") for f in factors):
                raise ValueError(
                    f"Transformation input does not match an event variable: {pattern}"
                )
            if (
                not matches
                and not any(c in pattern for c in "*?[")
                and operation not in {"Convolve", "Select"}
            ):
        ...
        if operation in {"Scale", "Demean"}:
            ...
            indices = (
                list(events.groupby(groups, dropna=False).indices.values())
                if groups
            ...
            for name in names:
                ...
                for rows in indices:
                    ...
                    if operation == "Scale" and len(np.unique(source)) < 2:
                        raise ValueError(
                            f"Cannot Scale a constant event group: {name}; use Demean for centering only"
                        )
                    transformed = (
                        source - source.mean() if instruction.get("Demean", True) else source.copy()
                    )
