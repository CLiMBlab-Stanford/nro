"""Run ICA-AROMA denoising with nro-managed inputs and outputs."""
...
def _resource_path(name: str) -> Path:
    return Path(resources.files("nro.modules.func.resources.ica_aroma").joinpath(name))
...
def _fslinfo_value(
    run_out: RunOutCommand, fsl_cmds: Mapping[str, str], img: Path, key: str
) -> float:
...
def cross_correlation(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    """Return every column-wise correlation between two sample matrices."""
...
def feature_time_series(melmix: Path, mc: Path) -> np.ndarray:
    """Compute ICA-AROMA's maximum motion-correlation feature."""
    import random
...
def feature_frequency(mel_ftmix: Path, tr: float) -> np.ndarray:
    """Compute ICA-AROMA's normalized high-frequency-content feature."""
    fs = 1.0 / tr
    nyquist = fs / 2.0
    ...
    if fincl.size == 0:
        raise RuntimeError(
            f"MELODIC frequency spectrum contains no frequencies above 0.01 Hz: {mel_ftmix}"
        )
    ft = ft[fincl, :]
    f = f[fincl]
...
def feature_spatial(
    *,
    ...
    """Compute ICA-AROMA edge and CSF spatial-fraction features."""
    try:
        import nibabel as nib  # type: ignore
...
def classification(
    ...
    edge_fract: np.ndarray,
    hfc: np.ndarray,
    csf_fract: np.ndarray,
) -> np.ndarray:
    """Classify motion components and write their feature tables."""
    thr_csf = 0.10
    thr_hfc = 0.35
    ...
    motion_ics = np.squeeze(
        np.array(np.where((proj > 0) + (csf_fract > thr_csf) + (hfc > thr_hfc)))
    )

    np.savetxt(
        out_dir / "feature_scores.txt", np.vstack((max_rp_corr, edge_fract, hfc, csf_fract)).T
    )
...
def denoising(
    *,
    ...
    """Apply aggressive or nonaggressive regression of motion components."""
...
def run_ica_aroma_workflow(
    *,
    ...
    """Run the complete in-repository ICA-AROMA implementation."""
    if tr <= 0:
