...
def clustering_membership(
    ...
    for repetition in range(config.repetitions):
        seed = None if config.random_seed is None else int(config.random_seed) + repetition
        LOG.info(
        ...
        if np.unique(labels).size != config.n_networks:
            raise ValueError("Mini-batch k-means returned fewer networks than requested")
