"""Write labeled parcellations and parcel connectomes as CIFTI images."""
...
def write_dlabel(
    ...
    """Write surface microparcel labels and return their CIFTI parcel axis."""
    nib = _nib()
    brain_axis, parcel_axis, names = _axes(labels, vertex_counts, surface_paths)
    table = {0: ("unassigned", (0.0, 0.0, 0.0, 0.0))}
    table.update(
        {index: (str(name), _parcel_color(index)) for index, name in enumerate(names, start=1)}
    )
...
def write_volume_dlabel(path: Path, labels: np.ndarray, mask: np.ndarray, affine: np.ndarray):
    ...
    brain_axis = nib.cifti2.BrainModelAxis.from_mask(mask, name="all_grey_matter", affine=affine)
    names = _parcel_names(count)
...
def _quantize(correlations: np.ndarray) -> np.ndarray:
    ...
    for start in range(0, correlations.shape[0], block_size):
        ...
        quantized[start:stop] = np.rint(np.clip(block, -1.0, 1.0) * PCONN_INT8_SCALE).astype(
            np.int8
        )
    return quantized
...
def write_pconn(path: Path, correlations: np.ndarray, parcel_axis) -> Path:
    """Write symmetric correlations as a scaled int8 CIFTI pconn."""
    nib = _nib()
    ...
    with Path(path).open("r+b") as file:
        ...
        nifti_header["scl_slope"] = 1.0 / PCONN_INT8_SCALE
