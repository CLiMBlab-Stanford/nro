"""Coarsen a local similarity graph into contiguous microparcels."""
...
def _smallest_eigenspace(
    lap: sparse.csr_matrix, k: int, tol: float
...
def _greedy_minimum_matching(
    edges: np.ndarray, costs: np.ndarray, merges_needed: int, n: int
) -> list[tuple[int, int]]:
...
def _coarsen_weights(w: sparse.csr_matrix, c: sparse.csr_matrix) -> sparse.csr_matrix:
    ...
    pinv = (
        normalized
        @ sparse.diags(
            np.divide(1.0, column_sums, out=np.zeros_like(column_sums), where=column_sums != 0)
        )
    ).T
...
def loukas_variation_edges(
    ...
    *,
    ...
    previous_c: sparse.csr_matrix | None = None
    ...
    for level in range(max_levels):
        ...
        if level == 0:
            ...
            reduced_basis = previous_c @ basis
