...
def discover_event_files(
    task: str, bids_root: Path, *, projects: Sequence[str] = (), participants: Sequence[str] = (),
) -> tuple[Path, ...]:
    ...
    """
    available = {path.name: path for path in bids_root.iterdir()
                 if path.is_dir() and any(path.glob("sub-*"))}
    ...
    if not paths:
        raise ValueError(f"No source BOLD runs for task {task!r}; supply --events FILE ... or --file FILE")
    return tuple(dict.fromkeys(paths))


def model_draft(
    paths: Sequence[Path], *, conditions: str | None = None,
    ...
    for name in sorted(columns):
        ...
        kinds = {"numeric" if pd.api.types.is_numeric_dtype(values) else "categorical" for values in present}
        report(f"  {name}: {'/'.join(sorted(kinds))}; present in {len(present)}/{len(tables)} tables")
    if conditions is None:
        if "trial_type" in common:
            conditions = "trial_type"
        elif "trial_type" in columns:
            raise ValueError("trial_type is absent from some tables; narrow discovery or specify --conditions COLUMN")
        elif choose is not None and common:
            conditions = choose(tuple(sorted(common)))
        else:
            raise ValueError(f"Choose --conditions from the common event columns: {', '.join(sorted(common))}")
    ...
    for label in sorted(counts):
        if any(char in label for char in "*?["):
            raise ValueError(f"Condition label {label!r} contains selector wildcards; write a model explicitly")
        ...
        report(f"  Condition {label!r}: {counts[label]} events, {presence[label]}/{len(tables)} tables; contrast {unique}")
    if missing:
        report(f"Warning: {missing} events have no condition label and receive no condition predictor.")
    report("Other event columns were not added as predictors. Review comparisons, baseline, and HRF before saving.")
    model = {"model_set": [], "conditions": conditions, "hrf": "spm", "contrasts": contrasts,
