...
def read_timeseries(paths: tuple[Path, ...]) -> tuple[np.ndarray, dict]:
    ...
    return np.concatenate(arrays, axis=1), {"domain": "surface", "counts": [a.shape[1] for a in arrays]}


def write_statmaps(prefix: Path, maps: dict[str, np.ndarray], geometry: dict,
    ...
    for statistic, values in maps.items():
        if geometry["domain"] == "volume":
            ...
            image = nib.Nifti1Image(np.asarray(values, np.float32).reshape(geometry["shape"]), reference.affine, header)
            ...
            for hemisphere, count in zip(("L", "R"), geometry["counts"]):
                array = nib.gifti.GiftiDataArray(np.asarray(values[start:start + count], np.float32))
                targets.append((prefix.with_name(f"{prefix.name}_hemi-{hemisphere}_stat-{statistic}_statmap.shape.gii"),
        ...
        for path, image in targets:
            ...
            sidecar = path.with_name(path.name.removesuffix(".nii.gz").removesuffix(".shape.gii") + ".json")
...
def load_fit(record: dict, spatial_slice: slice = slice(None)) -> RunFit:
    ...
    arrays = {name: np.load(path, mmap_mode="r", allow_pickle=False) for name, path in record["arrays"].items()}
    return RunFit(arrays["beta"][:, spatial_slice], arrays["residual_variance"][spatial_slice],
                  arrays["groups"][spatial_slice], arrays["covariance"], record["dof"], arrays["ar_coefficients"])
