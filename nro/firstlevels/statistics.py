...
def aggregate(estimates: list[Estimate], fits: dict[str, RunFit], *, weighting: str = "equal") -> Estimate:
...
def nuisance_components(task: np.ndarray, nuisance: np.ndarray, *,
                        variance_target: float, minimum_rank: int,
                        minimum_fraction: float) -> tuple[np.ndarray, dict]:
    ...
    if scaled.shape[1]:
        ...
        requested = 0
        explained = 0.0
    return components, {"ExactRank": int(exact_rank), "PostExactRank": remaining,
                        "NuisancePCs": components.shape[1], "RequestedNuisancePCs": requested,
                        "NuisanceVarianceExplained": explained, "NuisanceRankCap": cap}


def fit_glm(data: np.ndarray, design: np.ndarray, *, retained: np.ndarray,
            ar_grid: np.ndarray, block_size: int = 2048) -> RunFit:
