...
def validate_task_model(value: dict) -> dict:
    ...
    allowed = {"description", "model_set", "conditions", "predictors", "hrf",
               "hrf_overrides", "transformations", "contrasts", "statsmodels", "aggregation"}
...
def model_memberships(model: dict) -> tuple[str, ...]:
    ...
    if not isinstance(sets, list) or any(not isinstance(v, str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*", v) for v in sets):
...
def scientific_model(model: dict) -> dict:
    ...
    if not isinstance(model, dict):
        raise ValueError("Task model must be a YAML mapping")
    scientific = {key: value for key, value in model.items() if key not in {"model_set", "description"}}
...
def _canonical_model(serialized: str) -> dict:
    ...
    return {"predictors": rules["event_predictors"], "hrf": rules["default_hrf"],
...
def _selected_models(*, tasks=(), models=(), model_sets=None, root: Path | None = None) -> dict[str, dict]:
...
def select_models(*, tasks=(), models=(), model_sets=None, root: Path | None = None) -> dict[str, dict]:
    ...
    """
    return {identifier: validate_task_model(model) for identifier, model in
            _selected_models(tasks=tasks, models=models, model_sets=model_sets, root=root).items()}
