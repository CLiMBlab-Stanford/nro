...
NUISANCE_KEYS = ("confounds_regex", "temporal_mask_regex", "nuisance_variance_explained",
                 "minimum_temporal_rank", "minimum_temporal_rank_fraction", "noise_model", "ar_grid")
...
def canonical_model_document(document: dict) -> dict:
    ...
    for node in result["Nodes"]:
        if "Transformations" in node:
            ...
            transforms["Instructions"] = [canonical_transform(item) for item in transforms["Instructions"]]
...
def task_node(source: dict) -> dict:
    ...
    if not isinstance(predictors, list) or not predictors or any(not isinstance(v, str) or not v for v in predictors):
    ...
    if not isinstance(overrides, dict) or any(not isinstance(k, str) or v not in {"spm", "glover", None} for k, v in overrides.items()):
    ...
    if "Transformations" in advanced and ("transformations" in source or conditions):
        raise ValueError("Advanced Transformations cannot be combined with conditions or transformations")
    transforms = advanced.get("Transformations", {
        "Transformer": "pybids-transforms-v1",
        "Instructions": ([{"Name": "Factor", "Input": conditions}] if conditions else []) + source.get("transformations", []),
    })
    ...
    if "Contrasts" in advanced:
        ...
        for name, weights in declared.items():
            ...
            names = [f"{conditions}.{v}" if conditions and not v.startswith(f"{conditions}.") else v for v in weights]
            contrasts.append({"Name": name, "ConditionList": names, "Weights": list(weights.values()), "Test": "t"})
    node = {"Name": "run", "Level": "Run",
            "GroupBy": ["subject", "session", "task", "run", "direction", "acquisition"],
            "Transformations": deepcopy(transforms), "Model": {"Type": "glm", "X": [*predictors, 1],
            "Software": {"nro": {"event_predictors": predictors, "default_hrf": hrf,
                                   "hrf_overrides": overrides}}}, "Contrasts": contrasts}
    validate_model({"Name": "task", "BIDSModelVersion": "1.0.0", "Input": {"task": "task"}, "Nodes": [node]}, task="task")
...
def compile_model(source: dict, identifier: str, config: dict, *, sessions: bool = True) -> dict:
    ...
    primitives = list(dict.fromkeys(v for c in node["Contrasts"] for v, w in zip(c["ConditionList"], contrast_weights(c)) if w))
    ...
    node["Contrasts"].extend({"Name": name, "ConditionList": [p], "Weights": [1], "Test": "pass"}
                             for p, name in primitive_names.items())
    summary_contrasts = [{**c, "ConditionList": [primitive_names[p] for p, w in zip(c["ConditionList"], contrast_weights(c)) if w],
    ...
    for level in (["Session"] if sessions else []) + ["Subject"]:
        ...
        nodes.append({"Name": name, "Level": level,
                      "GroupBy": ["subject", "session"] if sessions and level == "Session" else ["subject"],
                      "Model": {"Type": "meta", "X": list(primitive_names.values()),
                                "Software": {"nro": {"aggregation_weighting": source["aggregation"]["weighting"]}}},
                      "Contrasts": deepcopy(summary_contrasts)})
        edges.append({"Source": "run", "Destination": name, "Filter": {"contrast": list(primitive_names.values())}})
    return canonical_model_document(validate_model({"Name": identifier.replace("/", "-"), "BIDSModelVersion": "1.0.0",
                           "Input": {"task": identifier.split("/")[0]}, "Nodes": nodes, "Edges": edges},
...
def realize_run_node(node: dict, events, confounds, config: dict) -> dict:
    ...
    predictors = expand_columns(rules["event_predictors"], list(sparse), factor_variables=tuple(factors))
    ...
    for name in predictors:
        matching = [value for pattern, value in rules.get("hrf_overrides", {}).items() if fnmatch.fnmatchcase(name, pattern)]
        if len(matching) > 1:
    ...
    nuisance = [name for name in confounds if name not in outliers and re.search(denoising["confounds_regex"], name)]
