...
def refresh_command(command: tuple[str, ...], processing: dict) -> tuple[str, ...]:
    ...
    if "--model-definition" in values and "task_model" in processing:
        values[values.index("--model-definition") + 1] = json.dumps(processing["task_model"], sort_keys=True)
    return tuple(values)
...
def selected_runs(runs, model_id: str, participant: str) -> tuple:
    ...
    return tuple(run for run in runs if run.entities.get("task") == task and run.participant == participant)


def select_model_runs(runs, config: dict, participant: str, *, entities: dict | None = None) -> tuple:
...
def plan_instances(context, upstream, descriptor) -> tuple[InstanceSpec, ...]:
    ...
    for model_id, document in context.task_models.items():
        ...
        for space, smoothing in context.target_pairs:
            ...
            entities = {"task": model_id.split("/")[0], "model": model_id.split("/")[1],
                        "space": space, "smoothing": str(smoothing)}
            result.append(InstanceSpec.create(
                key=instance_key(context.project, descriptor.name, context.registered.lineage_fingerprints["firstlevels"], context.participant, entities),
                module=descriptor.name, project=context.project, participant=context.participant, entities=entities,
                scope=descriptor.scope, configuration_lineage_id=context.registered.lineages["firstlevels"],
                config_fingerprint=context.workflow.configuration("firstlevels").scientific_fingerprint,
                directory_label=directory, runtime_config=context.runtime_config("firstlevels"),
                command=(sys.executable, "-m", "nro.firstlevels", "-p", context.participant, "-P", context.project,
                         "--model", model_id, "--model-definition", json.dumps(source, sort_keys=True),
                         "-s", space, "-S", str(smoothing)),
                dependencies=tuple(dict.fromkeys([*(instance.key for instance in upstream["anat"]), *(func[run.stem].key for run in runs)])),
                input_paths=direct_inputs(runs, values, context.participant, entities),
                output_root=root, output_prefix=prefix, output_format=descriptor.output_format,
                resource_class=descriptor.resource_class, memory_gb=context.memory_gb, max_memory_gb=context.max_memory_gb,
