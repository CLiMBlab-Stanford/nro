...
class EventFile:
    ...
    def snapshot(self) -> tuple[str, dict]:
        ...
        return text, {'catalog_id': self.identifier, 'sha256': hashlib.sha256(text.encode('utf-8')).hexdigest()}
...
class EventStore:
    ...
    def _entries(self, index: Path) -> tuple[list[str], list[EventFile]]:
        ...
        if set(value) != {'tasks', 'files'} or not isinstance(value['tasks'], list) or not all(isinstance(t, str) and task_key(t) for t in value['tasks']):
        ...
        for variant, entry in value['files'].items():
            ...
            if not isinstance(entry['path'], str) or not isinstance(entry['source_names'], list) or not all(isinstance(name, str) for name in entry['source_names']):
                raise ValueError(f'Event path and source names must be strings: {index}')
            relative = Path(entry['path'])
            path = (self.root / relative).resolve()
            if relative.is_absolute() or '..' in relative.parts or not path.is_relative_to(self.root) or path.suffix != '.tsv' or not path.is_file():
