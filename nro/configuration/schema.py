...
class Field:
    ...
    def normalize(self, value, location: str):
        ...
        if self.kind in {"int", "float"}:
            ...
            value = [self.item.normalize(item, f"{location}[{index}]") for index, item in enumerate(value)]
...
DENOISING = {
    "confounds_regex": Field("regex"), "temporal_mask_regex": Field("regex"),
    "nuisance_variance_explained": POSITIVE_FRACTION,
    "minimum_temporal_rank": NONNEGATIVE_INT, "minimum_temporal_rank_fraction": FRACTION,
}
CONTAINER = {
    "image": OPTIONAL_TEXT, "engine": TEXT, "cleanenv": BOOL, "bind": STRINGS,
    "home": OPTIONAL_TEXT, "inner_setup": SETUP, "no_container": BOOL,
}
THREADS = {"nthreads": EXEC_COUNT, "nthreads_divisor": EXEC_COUNT, "nthreads_min": EXEC_COUNT,
           "force": EXEC_BOOL, "verbose": EXEC_BOOL}

SCHEMAS = {
    "preprocessing": {
        "container": CONTAINER,
        "anat": {**THREADS, "selection_strategy": enum("first", "robust_average"),
                 "mni_template": TEXT, "synthstrip_container": TEXT,
                 "freesurfer_subjects_dir": OPTIONAL_TEXT, "fs_subject": OPTIONAL_TEXT},
        "func": {**THREADS, "sdc_method": enum("syn", "synbold_disco"),
                 "synbold_disco_image": TEXT, "synbold_disco_license": TEXT,
                 "bbregister_surf": enum("white", "pial"), "bbregister_init": enum("coreg", "fsl", "header", "rr"),
                 "bbregister_dof": Field("int", choices=(6, 9, 12)), "output_grid": enum("t1_native", "t1_epi_vox"),
                 "topup_config": TEXT, "ica_aroma_cmd": OPTIONAL_TEXT, "use_jacobian": BOOL,
                 "fieldmap_syn_refine": BOOL, "synbold_overlap_erosion_voxels": NONNEGATIVE_INT,
                 "synbold_min_overlap_voxels": COUNT, "synbold_max_rigid_translation_mm": POSITIVE,
                 "synbold_max_rigid_rotation_degrees": POSITIVE, "sbref_max_rigid_displacement_mm": POSITIVE,
                 "sbref_max_rigid_rotation_degrees": POSITIVE, "sbref_min_support_overlap": FRACTION,
                 "sbref_min_intensity_correlation": Field("float", minimum=-1, maximum=1),
                 **{f"syn_{stage}_{key}": TEXT for stage in ("base", "refine")
                    for key in ("transform", "convergence", "shrink_factors", "smoothing_sigmas")},
                 "clean_ica_aroma": BOOL, "ica_aroma_denoise_type": enum("nonaggr", "aggr", "both"),
                 "sdc_from_sbref_pair": BOOL, "debug_first_nvols": NONNEGATIVE_INT,
                 "io_chunk_vols": EXEC_COUNT, "output_spaces": Field("list", item=TEXT, nonempty=True)},
        "confounds": {"aseg_in_epi": OPTIONAL_TEXT, "brain_mask_in_epi": OPTIONAL_TEXT,
                      "n_acompcor": NONNEGATIVE_INT, "acompcor_max_voxels": COUNT, "fd_radius_mm": POSITIVE,
                      "motion_outlier_fd_thresh": POSITIVE, "nonsteady_max_vols": NONNEGATIVE_INT,
                      "nonsteady_rel_thresh": FRACTION, "nonsteady_stable_run": COUNT},
    },
    "clean": {**DENOISING, "min_trs": COUNT, "gm_mask_threshold": FRACTION,
              "standardize": BOOL, "detrend": BOOL, "regress_out_task": BOOL,
              "low_pass": Field("float", nullable=True, minimum=0, exclusive_minimum=True),
              "high_pass": Field("float", nullable=True, minimum=0), "force": EXEC_BOOL, "verbose": EXEC_BOOL,
              "container": OPTIONAL_TEXT, "no_container": BOOL, "container_engine": TEXT,
              "container_cleanenv": BOOL, "container_bind": STRINGS, "container_home": OPTIONAL_TEXT,
              "container_inner_setup": SETUP, "wb_command": TEXT},
    "firstlevels": {**DENOISING, "noise_model": enum("ols", "ar1"),
                    "ar_grid": Field("list", item=Field("float", minimum=-1, maximum=1,
                                                        exclusive_minimum=True, exclusive_maximum=True), nonempty=True),
                    "spatial_block_size": EXEC_COUNT, "wb_command": TEXT},
    "microparcellation": {
        "input_filter": Field("filter"), "surface": enum("pial", "midthickness", "white", "inflated"),
        "mask": OPTIONAL_TEXT, "mask_threshold": Field("float", minimum=0, maximum=1, exclusive_maximum=True),
        "volume_connectivity": Field("int", choices=(6, 18, 26)), "output_dir": OPTIONAL_TEXT,
        "prefix": OPTIONAL_TEXT, "overwrite": EXEC_BOOL,
        "coarsening": {"target_vertices": Field("int", minimum=2), "iterations": COUNT,
                       "exponential_temperature": POSITIVE, "eigenvectors": Field("int", minimum=2),
                       "max_levels": COUNT, "eigensolver_tolerance": NONNEGATIVE},
        "connectivity": {"minimum_retained_frames": Field("int", minimum=2), "minimum_retained_fraction": FRACTION,
                         "minimum_residual_design_dof": NONNEGATIVE_INT, "minimum_participation_effective_rank": NONNEGATIVE,
                         "maximum_dominant_temporal_variance_fraction": FRACTION, "minimum_usable_runs": COUNT,
                         "minimum_aggregate_retained_frames": Field("int", minimum=2), "temporal_block_size": EXEC_COUNT,
                         "reliability_weighting": BOOL, "reliability_vertex_block_size": EXEC_COUNT,
                         "global_signal_regression": BOOL},
        "quality": {"split_half_block_frames": COUNT, "null_parcellations": COUNT, "random_seed": NONNEGATIVE_INT,
                    "region_growing_attempts": COUNT, "connectome_power_iterations": COUNT},
    },
    "networks": {
        "output_dir": OPTIONAL_TEXT, "prefix": OPTIONAL_TEXT, "overwrite": EXEC_BOOL,
        "parcellation_strategy": enum("ica", "clustering", "oslom"),
        "connectivity": {"transform": enum("clip_positive", "absolute", "square"), "minimum_weight": NONNEGATIVE,
                         "percentile_cutoff": Field("float", nullable=True, minimum=0, maximum=100)},
        "ica": {"n_networks": Field("int", minimum=2), "random_seed": OPTIONAL_SEED, "max_iterations": COUNT,
                "tolerance": POSITIVE, "upper_quantile": POSITIVE_FRACTION, "svd_oversamples": COUNT,
                "svd_power_iterations": NONNEGATIVE_INT},
        "clustering": {"n_networks": Field("int", minimum=2), "repetitions": COUNT, "random_seed": OPTIONAL_SEED,
                       "n_init": COUNT, "max_iterations": COUNT, "batch_size": COUNT,
                       "max_no_improvement": Field("int", nullable=True, minimum=1), "reassignment_ratio": FRACTION},
        "oslom": {"executable": OPTIONAL_TEXT, "initialization": enum("none", "leiden", "file"), "initial_partition": OPTIONAL_TEXT,
                  "leiden_resolution": POSITIVE, "leiden_iterations": COUNT, "leiden_seed": OPTIONAL_SEED,
                  "weighted": BOOL, "directed": Field("bool", choices=(False,)),
                  "significance": Field("float", minimum=0, maximum=1, exclusive_minimum=True, exclusive_maximum=True),
                  "repetitions": COUNT, "internal_runs": COUNT, "hierarchical_runs": NONNEGATIVE_INT,
                  "extra_args": STRINGS, "timeout_seconds": Field("float", nullable=True, minimum=0,
                                                                  exclusive_minimum=True, execution=True)},
        "consensus": {"assignment_threshold": FRACTION, "homeless_threshold": FRACTION, "minimum_match_jaccard": FRACTION},
        "labeling": {"enabled": BOOL, "candidates_per_reference": COUNT},
    },
}

RUNTIME_FIELDS = {
    "preprocessing": {}, "clean": {"preprocessing_directory": TEXT},
...
def normalize_fields(schema: dict, values: dict, *, location: str = "configuration", complete: bool = True) -> dict:
    ...
    if unknown:
        raise DefinitionError(f"{location}: Unknown configuration option: {', '.join(sorted(unknown))}")
    ...
    for key, value in values.items():
        rule = schema[key]
        name = f"{location}.{key}"
        result[key] = (normalize_fields(rule, value, location=name, complete=complete)
                       if isinstance(rule, dict) else rule.normalize(value, name))
    return result
...
def _relationships(kind: str, values: dict) -> None:
    if kind == "clean":
        ...
        if connectivity["reliability_weighting"] and connectivity["minimum_retained_frames"] < 8:
            raise DefinitionError("connectivity.minimum_retained_frames must be at least 8 for quarter-split reliability")
    elif kind == "networks":
        oslom = values["oslom"]
        if values["parcellation_strategy"] == "oslom" and oslom["initialization"] == "file" and oslom["initial_partition"] is None:
            raise DefinitionError("oslom.initial_partition is required when oslom.initialization is file")
    ...
    def select(schema, mapping):
        ...
        for key, value in mapping.items():
            ...
            if isinstance(rule, Field) and rule.execution:
                continue
            result[key] = select(rule, value) if isinstance(rule, dict) and isinstance(value, dict) else deepcopy(value)
        return result
    schema = SCHEMAS[kind]
    if kind in {"microparcellation", "networks"}:
        schema = {**schema, "output": {"overwrite": EXEC_BOOL, "work_directory": Field("str", execution=True)}}
