...
def validate_store(root: Path | None = None) -> dict[str, int]:
    ...
    from nro.firstlevels.task_models import load_task_model, scientific_model
    ...
    for path in files['configs']:
        ...
        suffix = f"_{kind}.yml"
        if len(relative.parts) != 2 or kind not in DERIVATIVE_CLASSES or not path.name.endswith(suffix):
...
def create_store(root: Path | None = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix='.nro-definitions-', dir=destination.parent) as temporary:
