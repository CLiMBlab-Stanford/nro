...
from nro.orchestration.runtime import load_runtime_workflow_snapshot
...
def main(argv: list[str] | None = None) -> None:
    ...
    runtime_config = select_runtime_config(
        project=args.project, workflow_id=args.workflow, derivative_class="clean"
    )
    ...
    source_subject = Path(BIDS_PATH) / args.project / sub_id
    try:
    ...
    from nro.clean.module import main as run_clean

    module_argv = [
        "--project", args.project,
        "--preprocessing-id", cfg["preprocessing_directory"],
        "--clean-id", clean_id,
        "--sub-id", sub_id,
        "--run-stem", run_stem,
    ...
    return_code = run_clean(module_argv)
    if return_code:
