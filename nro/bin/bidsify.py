...
from nro.bidsify.config import load_config, bids_label, identifier
from nro.bidsify.flywheel import FlywheelSource
from nro.bidsify.discovery import session_choices, inferred_session
from nro.bidsify.identity import destination_label
from nro.bidsify.store import IngestionStore, ReviewBusyError
from nro.bidsify.review import ask, wizard, choose_indices, SkipSession
...
def build_parser(*, prog='nro bidsify'):
    ...
    parser.add_argument('--rebidsify', action='store_true', help='Include sessions already in BIDS, regardless of producer; replacement still needs approval')
    parser.add_argument('--no-submit', action='store_true', help='Save work without supplying new workers')
    parser.add_argument('--bids-root', type=Path, default=BIDS_PATH)
    return parser


def select_source(config: dict, project: str, *, server: str | None,
                  flywheel_project: str | None) -> tuple[str, str]:
    """Select one server/project pair from the destination's allowed sources."""
    sources = config['project_sources'].get(project)
    if sources is None:
        sources = [dict(server=name, project=path) for name, profile in config['servers'].items()
                   for path in profile['projects']]
    choices = list(dict.fromkeys((s['server'], s['project']) for s in sources
                   if (server is None or s['server'] == server)
                   and (flywheel_project is None or s['project'] == flywheel_project)))
    if not choices:
        raise ValueError(f'No source configured for BIDS project {project} matches the requested server/Flywheel project')
    if len(choices) == 1:
        return choices[0]
...
def advance(store: IngestionStore, record: dict) -> dict:
    ...
    if record['state'] in {'running', 'published', 'cancelled'} or (record['state'] == 'queued' and record['executor_uid'] == os.getuid()):
...
def _advance(store: IngestionStore, record: dict, *, review_token: str) -> dict:
    ...
    if record['state'] == 'queued':
        if record['executor_uid'] != os.getuid() and ask('Execute this request using your authenticated account? y/N', 'n').lower() == 'y':
    ...
    if record['state'] in {'failed', 'interrupted'}:
        ...
        if action == 'r':
            ...
            record.update(state='needs_input', stage='convert' if all('metadata' in a or a['datatype'] == 'ignore' for a in record['acquisitions']) else 'prepare', approval=None)
        elif action == 'c':
            record.update(state='cancelled')
        else:
            return record
        record = store.update(record, expected_revision=record['revision'], review_token=review_token)
    if record['state'] == 'needs_input':
        return wizard(store, record, review_token=review_token)
    if record['state'] == 'awaiting_approval':
        snapshot = approval_snapshot(record, store.registry)
        print('Staged outputs (SHA256):')
        for name, digest in snapshot['outputs'].items():
            print(f'  {name}  {digest}')
        if snapshot['target_exists']:
            print('Replacement removes the current session after atomic exchange. Stop external readers first.')
            for name in snapshot['existing']:
                print('  Existing: ' + name)
        print('Validation passed. No unresolved acquisition or event assignments remain.')
        answer = ask('Approve publication of these exact files, edit decisions, or leave? y/e/N', 'n').lower()
        if answer == 'e':
...
def main(argv=None, *, prog='nro bidsify'):
    ...
    try:
        ...
        if not args.request and not config['servers']:
            raise ValueError('Configure Flywheel servers in the definitions store: bidsify/main.yml')
        registry.initialize()
        ...
        if args.request:
            ...
            server, remote_project = select_source(config, project, server=args.server,
                                                  flywheel_project=args.flywheel_project)
            all_records = store.rows()
            saved = [r for r in all_records if r['server'] == server and r['project'] == project
            ...
            if not records:
                ...
                discovery = session_choices(remote, server=server, bids_root=args.bids_root,
                    rules=config['session_rules'], records=all_records,
                    rebidsify=args.rebidsify, sessions=args.session)
                choices = discovery.rows
                if discovery.existing_hidden:
                    print(f'Omitted {discovery.existing_hidden} sessions already bidsified; use --rebidsify to include them.')
                if discovery.active_hidden:
                    print(f'Omitted {discovery.active_hidden} sessions with unfinished nro requests; use nro status to locate them.')
                for number, row in enumerate(choices, 1):
                    label = ('ambiguous BIDS mapping; review required' if row['mapping_ambiguous'] else
                             'already bidsified' if row['already_bidsified'] else 'no known BIDS match')
                    print(f"{number}: {row['id']} {row['label']} ({row['remote_project']}; {label})")
                ...
                for index in indices:
                    ...
                    if selected['mapping_ambiguous']:
                        print('Existing destination is ambiguous; resolve the session rules before re-bidsification.')
                        continue
                    existing = selected['existing_bids'][0] if selected['existing_bids'] else None
                    if existing and existing.project != project:
                        raise ValueError(f'This session already belongs to BIDS project {existing.project}; '
                                         're-bidsification cannot reassign it')
                    prior = next((r for r in reversed(saved) if r['remote_session'] == selected['id']), None)
                    answer = ask('BIDS participant label; Enter to leave pending',
                                 existing.participant if existing else
                                 (prior['participant'] or '') if prior else (args.participant[0] if len(args.participant) == 1 else ''))
                    ...
                    session = session or inferred_session(selected, server=server, rules=config['session_rules'])
                    if session:
                        print(f'BIDS session: ses-{session}')
                    else:
                        answer = ask('BIDS session label could not be inferred; enter a label or leave pending')
                        session = bids_label(answer) if answer else None
                    if existing and (participant, session) != (existing.participant, existing.session):
                        raise ValueError('Re-bidsification must retain the existing participant/session destination')
                    destination = destination_label(dict(project=project, participant=participant, session=session))
                    if ask(f'Queue inspection for {destination}? y/N', 'n').lower() != 'y':
                        continue
                    records.append(store.create(server=server, remote_session=selected['id'], project=project,
                                                participant=participant, session=session, config=config, replace=args.rebidsify))
        ...
        if queued and not args.no_submit:
            ...
            script = _write_worker_script(registry, bids_root=args.bids_root.resolve(), partition=site['partition'],
                account=site['account'], hours=max(r['config']['hours'] for r in queued), memory_gb=memory,
