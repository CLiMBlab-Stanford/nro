...
class AssessmentSnapshot:
    ...
    Rows include the selected work items and all their ancestors. Mutable worker
    heartbeats and resource limits are excluded from the comparison token.
    This record grants no authority to update the represented work items.
    """
...
def _capture_locked(registry, db, *, work_item_ids=None, projects=None) -> AssessmentSnapshot:
    ...
    edges = [
        ...
        for row in db.execute(
            "SELECT * FROM work_item_dependencies ORDER BY work_item_id, upstream_work_item_id, role"
        )
    ]
    requested = None if work_item_ids is None else set(work_item_ids)
    ...
    parents = {}
    for edge in edges:
        parents.setdefault(edge["work_item_id"], set()).add(edge["upstream_work_item_id"])
    ...
    lineages = {row["module_lineage_id"] for row in work_items}
    configs = tuple(
        dict(row)
        for row in db.execute(
            "SELECT id, config_fingerprint, resolved_yaml FROM module_lineages ORDER BY id"
        )
    ...
    attempts = tuple(
        dict(row)
        for row in db.execute(
            "SELECT id, work_item_id, state FROM attempts WHERE state IN "
            "('queued', 'running', 'cancel_requested') ORDER BY id"
        )
        if row["work_item_id"] in selected
    )
    mutations = tuple(
        dict(row)
        for row in db.execute("SELECT * FROM artifact_mutations ORDER BY work_item_id")
        if row["work_item_id"] in selected
    )
...
def capture_assessment(
    registry, *, work_item_ids: Iterable[int] | None = None, projects: Iterable[str] | None = None
) -> AssessmentSnapshot:
    """Read selected records and their dependency closure in one locked transaction."""
    with registry.connection() as db:
        return _capture_locked(registry, db, work_item_ids=work_item_ids, projects=projects)
...
def apply_assessment(
    ...
    selected = {row["id"] for row in snapshot.work_items}
    if (
    ...
    now = utcnow()
    by_id = {row["id"]: row for row in snapshot.work_items}
    for update in report.updates:
        if update["contract"] is not None:
            ...
            if fixed(original) != fixed(update["contract"]):
                raise ValueError(
                    "Assessment cannot change work-item identity, dependencies, or output paths"
                )
    ...
    with registry.connection(write=True) as db:
        current = _capture_locked(registry, db, work_item_ids=selected)
        ...
        for update in report.updates:
            ...
            if update["contract"] is not None:
                ...
                db.execute(
                    """UPDATE work_items SET artifact_state=?, artifact_reason=?,
                ...
                db.execute(
                    """UPDATE attempts SET state='cancel_requested', error_type='WorkItemGraphChanged',
            ...
            if update["state"] == "fresh":
                db.executemany(
                    ...
                    [
                        (
                            by_id[edge["upstream_work_item_id"]]["current_generation"],
                            work_item_id,
                            edge["upstream_work_item_id"],
                        )
                        for edge in snapshot.dependencies
                        if edge["work_item_id"] == work_item_id
                    ],
                )
        dependency_state.synchronize(db, now=now)
        return {
            row["id"]: (row["artifact_state"], row["artifact_reason"])
            for row in db.execute("SELECT id, artifact_state, artifact_reason FROM work_items")
            if row["id"] in selected
        }
