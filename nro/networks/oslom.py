...
def write_oslom_graph(path: Path, adjacency: sparse.spmatrix) -> int:
    ...
    with path.open("w", encoding="utf8") as f:
        for block_index, start in enumerate(
            range(0, lower.shape[1], columns_per_block), start=1
        ):
...
def run_oslom(graph: Path, workdir: Path, cfg: OslomConfig, *, runner: Runner) -> list[set[int]]:
    ...
    cmd = [str(cfg.executable), "-f", local_graph.name, "-t", str(cfg.significance), "-r", str(cfg.internal_runs), "-hr", str(cfg.hierarchical_runs)]
    if cfg.weighted:
        cmd.append("-w")
    else:
        cmd.append("-uw")
    ...
    candidates = [workdir / "graph.dat_oslo_files" / "tp_without_singletons", workdir / "graph.dat_oslo_files" / "tp"]
