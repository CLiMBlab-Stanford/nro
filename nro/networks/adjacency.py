...
from nro.microparcellation.cifti import INT8_SCALE
...
def pconn_to_adjacency(
    ...
    *,
    ...
    if not np.isclose(slope, 1.0 / INT8_SCALE) or not np.isclose(intercept, 0.0):
        raise ValueError(f"Unexpected pconn scaling in {path}: slope={slope}, intercept={intercept}")
    ...
    for column in sample_columns:
        ...
        if not np.array_equal(below, across):
            raise ValueError(f"Microparcel connectivity is not symmetric near column {column}: {path}")
    ...
    percentile_weight = None
    if percentile_cutoff is not None:
        percentile_weight = _histogram_percentile(
            transformed, raw_counts, percentile_cutoff
        )
