...
def selected_records(registry, selection) -> list[dict]:
    ...
    if selection.modules or selection.workflows or selection.models or selection.model_sets or selection.spaces or selection.smoothing:
        return []
    sessions = selection.runs.get('ses', ())
    if set(selection.runs) - {'ses'}:
        return []
    return [{k: r[k] for k in ('id', 'server', 'project', 'participant', 'session', 'state', 'stage', 'issues')}
            for r in IngestionStore(registry).rows()
...
def render(records: list[dict], *, color: bool = False) -> str:
    ...
    if not records:
        return ''
    lines = ['\nBidsification', f"{'PROJECT':14} {'PARTICIPANT':14} {'SESSION':20} {'STATE':20} NEXT ACTION"]
    for r in records:
        command = '—' if r['state'] in {'queued', 'running'} else f"nro bidsify --request {r['id']}"
        state = f"{r['state'].replace('_', ' '):20}"
        if color:
            tone = '91' if r['state'] in {'failed', 'interrupted'} else '93' if command != '—' else '96'
