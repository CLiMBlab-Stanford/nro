"""Noninteractive scheduled stages for one reviewed ingestion request."""
...
from .images import command, prepare_image, secure_directory
...
def prefix(record: dict, item: dict) -> str:
    ...
    entities = {'sub': record['participant'], 'ses': record['session'], **item['entities']}
    return '_'.join(f'{key}-{entities[key]}' for key in ('sub','ses','task','acq','dir','run','echo') if key in entities)
...
def convert(record: dict, registry) -> dict:
    ...
    for item in acquisitions.values():
        if item.get('sbref'):
            if item['sbref'] in sbref_owners:
                raise BidsificationError('A shared SBRef needs an explicit sharing representation; assign distinct references or none')
            sbref_owners[item['sbref']] = item
    destinations = {}
    for item in acquisitions.values():
        if item['datatype'] == 'ignore' or item['suffix'] == 'sbref' and item['id'] not in sbref_owners:
            continue
        selected = sbref_owners.get(item['id'], item)
        entities = '_'.join(f'{key}-{selected["entities"][key]}' for key in ('task','acq','dir','run','echo') if key in selected['entities'])
        description = {'datatype': item['datatype'], 'suffix': item['suffix'], 'criteria': {'SidecarFilename': 'image.json'}}
        if entities:
        ...
        Dcm2BidsGen(dicom_dir=[str(helper)], participant=record['participant'], session=record['session'],
                    config=str(config), output_dir=str(bids), skip_dcm2niix=True, force_dcm2bids=True).run()
        directory = bids / f"sub-{record['participant']}" / f"ses-{record['session']}" / item['datatype']
        expected = directory / f"{prefix(record, selected)}_{item['suffix']}.nii.gz"
        if not expected.is_file():
            raise BidsificationError('dcm2bids did not produce the approved filename; publication stopped')
        sidecar = expected.with_name(expected.name[:-7] + '.json')
        metadata = json.loads(sidecar.read_text())
        if item['suffix'] == 'bold':
            metadata['NROReferencePolicy'] = 'explicit'
            metadata['NROSBRef'] = f"{prefix(record, selected)}_sbref.nii.gz" if item.get('sbref') else None
            metadata['B0FieldSource'] = ['nro' + min(item['fieldmaps']).replace('-', '')] if item.get('fieldmaps') else []
            if item['events']:
                shutil.copyfile(item['events'], directory / f"{prefix(record, selected)}_events.tsv")
    ...
    for pair, runs in pairs.items():
        ...
        for key in pair:
            ...
            metadata['IntendedFor'] = ['bids::' + str(destinations[r][0].relative_to(bids)) for r in runs]
            atomic_write_text(sidecar, json.dumps(metadata, indent=2))
    atomic_write_text(bids / 'dataset_description.json', json.dumps({'Name': record['project'], 'BIDSVersion': '1.11.0', 'DatasetType': 'raw'}))
    command([*record['config']['validator'], str(bids)])
    from importlib.metadata import version
    return {'state': 'awaiting_approval', 'issues': [], 'validation': 'passed',
            'software': {name: version(name) for name in ('dcm2bids', 'pydicom', 'nibabel')},
            'output_hashes': inventory(bids / f"sub-{record['participant']}" / f"ses-{record['session']}")}


def run_stage(record: dict, registry, *, source=None) -> dict:
    ...
    if record['stage'] == 'inspect':
        ...
        return {'state': 'needs_input', 'stage': 'prepare', 'acquisitions': acquisitions,
                'issues': ['Confirm acquisition classifications, BIDS entities, and task events before download']}
    if record['stage'] == 'prepare':
        unresolved = issues(record, prepared=False)
        if unresolved:
            return {'state': 'needs_input', 'issues': unresolved}
        runner = Runner(module_name='Bidsification preparation', container=None, binds=(),
                        logger=logging.getLogger('bidsify'), next_step=count(1).__next__)
        for item in record['acquisitions']:
            ...
            runner.add_step(Step.python(name=f"Prepare acquisition {item['id']}", action=action,
                                       outputs=(root / 'helpers' / item['id'] / '.prepared.json',), force=True))
        with runner.run_context():
            runner.execute()
        return {'state': 'needs_input', 'stage': 'convert', 'acquisitions': record['acquisitions'],
                'issues': issues(record, prepared=True)}
    if record['stage'] == 'convert':
        return convert(record, registry)
    if record['stage'] == 'publish':
        return {'state': 'published', **publish(record, registry)}
