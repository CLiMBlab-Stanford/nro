...
from .config import identifier, bids_label
...
class IngestionStore:
    ...
    def __init__(self, registry):
        """Bind the durable ingestion directory without creating files."""
        self.registry = registry
        self.root = registry.paths.control / "ingestion"
    ...
    def create(self, *, server: str, remote_session: str, project: str,
               participant: str | None = None, session: str | None = None,
               config: dict, replace: bool = False) -> dict:
        ...
        for value in (participant, session):
            if value is not None:
                bids_label(value)
        if Path(config['staging']).resolve().is_relative_to(self.registry.paths.bids_root.resolve()):
            raise ValueError('Shared ingestion staging must be outside the BIDS tree')
        with self.registry.connection(write=True):
            matches = [r for r in self.rows() if (r['server'], r['remote_session']) ==
                       (server, remote_session)]
            for previous in matches:
                if previous['project'] != project:
                    raise ValueError(f"This source session belongs to BIDS project {previous['project']}; "
                                     're-bidsification cannot reassign it')
                for field, value in (('participant', participant), ('session', session)):
                    if previous[field] is not None and value is not None and previous[field] != value:
                        raise ValueError('This source session already has a different BIDS destination')
            for field in ('participant', 'session'):
                known = {r[field] for r in matches if r[field] is not None}
                if len(known) > 1:
                    raise ValueError('This source session has conflicting registered BIDS destinations')
                if known:
            ...
            if unfinished:
                if any(value is not None and unfinished[-1][key] != value
                       for key, value in (('participant', participant), ('session', session))):
                    raise ValueError('Remote session already has a different unfinished BIDS mapping')
                return unfinished[-1]
            ...
            record = dict(id=request_id, server=server, remote_session=remote_session,
                          project=project, participant=participant, session=session,
                          config=deepcopy(config), state="queued", stage="inspect", revision=0,
                          executor_uid=os.getuid(), worker=None, issues=[], acquisitions=[],
                          replace=replace, approval=None, created=time.time())
    ...
    def _validate_destination_locked(self, record: dict) -> None:
        ...
        if any(r['id'] != record['id'] and
               all(r[key] == record[key] for key in ('project', 'participant', 'session'))
               and r['state'] not in {'published', 'cancelled'} for r in self.rows()):
            raise ValueError('Another unfinished ingestion request owns this destination')
        target = (self.registry.paths.bids_root / record['project'] /
                  f"sub-{record['participant']}" / f"ses-{record['session']}")
    ...
    def acquire_review(self, request_id: str, *, seconds: float = REVIEW_LEASE_SECONDS) -> str:
        ...
        with self.registry.connection(write=True):
            ...
            if lease and lease['expires'] > time.time():
                raise ReviewBusyError(f"Session is being reviewed by {lease['user']} on {lease['hostname']} (PID {lease['pid']}); skipped")
            if record['state'] in {'running', 'published', 'cancelled'}:
                raise ReviewBusyError(f"Session is {record['state']}; review skipped")
            token = uuid.uuid4().hex
            self._write_review(request_id, dict(token=token, user=getpass.getuser(),
                hostname=socket.gethostname(), pid=os.getpid(), expires=time.time() + seconds))
            return token

    def renew_review(self, request_id: str, token: str, *, seconds: float = REVIEW_LEASE_SECONDS) -> None:
    ...
    def update(self, record: dict, *, expected_revision: int, review_token: str,
               event_files: dict[str, str] | None = None) -> dict:
        ...
        with self.registry.connection(write=True):
            ...
            for key in ('id', 'server', 'remote_session', 'project', 'config'):
            ...
            if changed:
                if current['stage'] not in {'inspect', 'prepare', 'convert'} or current['state'] == 'awaiting_approval':
            ...
            for acquisition, text in (event_files or {}).items():
                ...
                if provenance and provenance.get('sha256') != digest:
                    raise ValueError('Event catalog provenance does not match the reviewed snapshot')
                destination = Path(current['config']['staging']) / current['id'] / 'events' / f'{identifier(acquisition)}-{digest}.tsv'
    ...
    def summary(self, memory_gb: int | None = None) -> tuple[int, int, int]:
        ...
        ready = sum(r["state"] == "queued" and r["executor_uid"] == os.getuid()
                    and not self._review_active(r['id'])
                    and (memory_gb is None or r["config"]["memory_gb"] <= memory_gb)
                    for r in rows)
        limit = max((r["config"]["concurrency"] for r in rows if r["state"] in ACTIVE), default=0)
    ...
    def claim(self, worker: str, memory_gb: int) -> dict | None:
        ...
        with self.registry.connection(write=True) as db:
            ...
            if db.execute("SELECT 1 FROM metadata WHERE key='maintenance_mode'").fetchone():
                return None
            active, _, limit = self.summary(memory_gb)
            if any(r['state'] == 'running' and r['worker'] == worker for r in self.rows()):
                return None
            limit = max(limit, db.execute("SELECT COALESCE(MAX(concurrency),0) FROM requests WHERE state='active'").fetchone()[0])
            active += db.execute("SELECT COUNT(*) FROM attempts WHERE state IN ('queued','running','cancel_requested')").fetchone()[0]
            if active >= limit:
                return None
            for row in self.rows():
                if row["state"] != "queued" or row["executor_uid"] != os.getuid() or row["config"]["memory_gb"] > memory_gb or self._review_active(row['id']):
                    continue
                row.update(state="running", worker=worker, hostname=socket.gethostname(), revision=row["revision"] + 1)
                self.write_locked(row)
                return row
        return None

    def finish(self, request_id: str, worker: str, *, state: str, changes: dict | None = None) -> None:
