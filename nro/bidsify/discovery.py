...
def validate_session_rules(rules: object, servers: dict) -> None:
    ...
    for rule in rules:
        if not isinstance(rule, dict) or set(rule) != {'server', 'remote_project', 'match', 'participant', 'session'}:
            raise ValueError('Session rules require server, remote_project, match, participant, and session')
        ...
        for field in ('participant', 'session'):
            ...
            for _, name, spec, conversion in Formatter().parse(template):
                if name is not None and (name not in groups or spec or conversion):
                    raise ValueError('BIDS templates may reference only named regex groups without formatting')
...
def existing_sessions(bids_root: Path) -> tuple[ExistingSession, ...]:
    ...
    for project in sorted(bids_root.iterdir()):
        if (not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9_-]*', project.name)
                or project.name in {'derivatives', 'sourcedata'} or not project.is_dir()):
            continue
...
def _rule_labels(remote: dict, rule: dict) -> tuple[str | None, str | None] | None:
    ...
    for field, pattern in rule['match'].items():
        ...
        for name, value in match.groupdict().items():
            ...
            groups[name] = value
    try:
        return tuple(bids_label(rule[field].format_map(groups)) if rule[field] is not None else None
                     for field in ('participant', 'session'))
    except ValueError:
        return None
...
def session_choices(remote: list[dict], *, server: str, bids_root: Path, rules: list[dict],
                    records: list[dict], rebidsify: bool = False, sessions=()) -> SessionChoices:
    ...
    for row in remote:
        ...
        if existing and not rebidsify:
            existing_hidden += 1
            continue
        choices.append({**row, 'existing_bids': matches, 'mapping_ambiguous': ambiguous,
