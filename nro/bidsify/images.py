...
from nro.engine.io import atomic_write_text, atomic_output_path
from .config import identifier


METADATA_FIELDS = {
    "RepetitionTime", "EchoTime", "EchoNumber", "FlipAngle", "MagneticFieldStrength",
    "PhaseEncodingDirection", "TotalReadoutTime", "EffectiveEchoSpacing", "SliceTiming",
    "MultibandAccelerationFactor", "ParallelReductionFactorInPlane", "ReconMatrixPE",
    "AcquisitionMatrixPE", "BandwidthPerPixelPhaseEncode", "InPlanePhaseEncodingDirectionDICOM",
    "ImageOrientationPatientDICOM", "PartialFourier", "SliceThickness", "SpacingBetweenSlices",
    "DwellTime", "InversionTime", "NumberOfVolumesDiscardedByScanner",
    "NumberOfVolumesDiscardedByUser", "AcquisitionTime", "SeriesNumber", "ImageType",
    "Manufacturer", "ManufacturersModelName", "ReceiveCoilName", "MRAcquisitionType",
}
...
def command(argv: list[str], *, env: dict | None = None) -> None:
    ...
    try:
        subprocess.run(argv, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=env)
    except (OSError, subprocess.CalledProcessError):
        raise BidsificationError(f"{Path(argv[0]).name} failed; no output has been approved for publication") from None
    ...
    for part in path.parts[1:]:
        ...
        if current.is_relative_to('/tmp/nro'):
            if info.st_mode & stat.S_IRWXO:
                raise BidsificationError('Anatomical staging must not be accessible outside its owner and group')
            if info.st_uid not in {0, os.getuid()} and info.st_gid not in {os.getgid(), *os.getgroups()}:
                raise BidsificationError('Anatomical staging is not owned by your user or a shared group')
    return path


def temporary_anatomy(record: dict) -> Path:
    """Resolve deterministic node-local anatomy staging without creating it."""
    return Path('/tmp/nro/bidsify') / identifier(record['server']) / identifier(record['remote_session']) / identifier(record['id'])
    ...
    if zipfile.is_zipfile(source):
        with zipfile.ZipFile(source) as archive:
            for index, member in enumerate(archive.infolist()):
                ...
                with archive.open(member) as src, (destination / f'{index:08d}.dcm').open('wb') as dst:
        ...
        with tarfile.open(source) as archive:
            for index, member in enumerate(archive):
                ...
                with archive.extractfile(member) as src, (destination / f'{index:08d}.dcm').open('wb') as dst:
                    shutil.copyfileobj(src, dst)
    else:
...
def sanitize_image(source: Path, destination: Path) -> None:
    ...
    if data.ndim not in (3, 4) or not np.isfinite(data).all() or not np.any(data):
        raise BidsificationError("Converted image is empty, nonfinite, or has unsupported dimensions")
...
def prepare_image(record: dict, item: dict, shared: Path, source) -> dict:
    ...
    source_identity = {k: item[k] for k in ('acquisition', 'file_token', 'bytes', 'datatype', 'suffix', 'source_revision')}
    if marker.is_file():
        saved = json.loads(marker.read_text())
        from .publication import file_hash
        if saved['source'] == source_identity and all((output / name).is_file() and file_hash(output / name) == digest for name, digest in saved['hashes'].items()):
            image = nib.load(output / 'image.nii.gz')
            if np.isfinite(np.asanyarray(image.dataobj)).all():
                return saved['metadata']
    anatomy = item['datatype'] == 'anat'
    root = temporary_anatomy(record) / identifier(item['id']) if anatomy else shared / 'raw' / identifier(item['id'])
    secure_directory(root)
    try:
        if shutil.disk_usage(root).free < max(item['bytes'] * 5, 100_000_000):
            raise BidsificationError("Insufficient staging space; no shared fallback for raw anatomy")
        raw = root / 'source'
        if raw.is_symlink():
        ...
        command([*converter, '-b', 'y', '-ba', 'y', '-z', 'y', '-f', 'converted', '-o', str(converted), str(dicoms)],
                env={**os.environ, 'TMPDIR': str(root)})
        images = list(converted.glob('*.nii.gz'))
        if len(images) != 1:
            raise BidsificationError("Acquisition produced multiple images; split the acquisition into explicit conversion selections")
        original = images[0]
        ...
        if anatomy:
            ...
            command([*stripper, '-i', str(original), '-o', str(stripped)], env={**os.environ, 'TMPDIR': str(root)})
        ...
        from .publication import file_hash
        atomic_write_text(marker, json.dumps({'source': source_identity, 'metadata': sanitized,
                                            'hashes': {name: file_hash(output / name) for name in ('image.nii.gz', 'image.json')}}), mode=0o660, durable=True)
        return sanitized
    finally:
        if root.exists():
            shutil.rmtree(root)
