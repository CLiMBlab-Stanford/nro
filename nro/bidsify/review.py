...
from nro.configuration.events import EventStore, EventFile
...
def event_candidates(config: dict, task: str) -> tuple[list[EventFile], list[str]]:
    ...
    paths = sorted({p for r in config['event_rules'] if r['task'] == task for p in glob.glob(r['pattern'])})
        ...
        if item['datatype'] == 'func' and item['suffix'] == 'bold':
            ...
            if task != 'rest':
                try:
                    ...
                    duration = shape[3] * metadata['RepetitionTime'] if len(shape) == 4 and 'RepetitionTime' in metadata else None
            ...
            if prepared:
                if 'sbref' not in item:
                    ...
                    if item['sbref'] in used_sbrefs:
                        unresolved.append(f'{name}: SBRef already assigned to another BOLD; select a distinct reference or none')
                ...
                if 'fieldmaps' not in item:
                    ...
                    if len(refs) != 2 or any(r is None or r['datatype'] != 'fmap' for r in refs) or not compatible(refs[0], refs[1], opposite=True) or not any(compatible(item, r) for r in refs):
                    ...
                    for key in pair:
                        if key in fieldmap_groups and fieldmap_groups[key] != pair:
                            unresolved.append(f'{name}: a fieldmap is assigned to conflicting pairs')
...
def _events(record: dict, item: dict) -> str | None:
    ...
    while True:
        answer = ask('Event TASK/VARIANT ID or TSV path; s to defer', item.get('events') or (defaults[0] if len(defaults) == 1 else ''))
        if answer == 's':
            return
        try:
            provenance = None
            if answer in choices or (len(answer.split('/')) == 2 and not answer.endswith('.tsv') and not answer.startswith(('.', '~', '/'))):
            ...
            print(str(error))
            continue
        if ask('Confirm event file contains no identifying columns or text? y/N', 'n').lower() == 'y':
            if provenance is not None:
                item['events_source'] = provenance
            elif item.get('events') != str(event_path) or item.get('events_source', {}).get('sha256') != hashlib.sha256(text.encode('utf-8')).hexdigest():
                item.pop('events_source', None)
            return text
        return


def _references(prompt: str, refs: list[dict], current: list[str], *, count: int) -> list[str] | None:
    for number, ref in enumerate(refs, 1):
        metadata = ref.get('metadata', {})
        print(f"{number}: {ref['id']} {ref['entities']} encoding={metadata.get('PhaseEncodingDirection')} time={metadata.get('AcquisitionTime')}")
    ...
    while True:
        ...
        try:
            ...
            if count == 2 and not compatible(refs[selected[0]], refs[selected[1]], opposite=True):
                raise ValueError('Select an opposite-encoding pair with matching geometry and readout time')
...
def wizard(store, record: dict, *, review_token: str) -> dict:
        ...
        for field in ('participant', 'session'):
            ...
            while True:
                ...
                try:
                    ...
                    record = store.update(candidate, expected_revision=record['revision'], review_token=review_token)
                    break
                except ValueError as error:
                    print(str(error))
    source = None
    ...
    for index in range(len(record['acquisitions'])):
        ...
        if prepared:
            ...
            refs = [a for a in record['acquisitions'] if a['suffix'] == 'sbref' and compatible(item, a)]
            answer = _references('SBRef number', refs, [item.get('sbref')], count=1)
            if answer is not None:
                item['sbref'] = answer[0] if answer else None
            refs = [a for a in record['acquisitions'] if a['datatype'] == 'fmap' and (compatible(item, a) or compatible(item, a, opposite=True))]
            answer = _references('Two fieldmap numbers separated by spaces', refs, item.get('fieldmaps', []), count=2)
            if answer is not None:
                item['fieldmaps'] = answer
        else:
            answer = ask('Type: anat/T1w, anat/T2w, func/bold, func/sbref, fmap/epi, ignore, or s',
            ...
            if kind[0] != 'ignore':
                print('Confirm classification before download: raw anatomy is permitted only in /tmp.')
                if ask('Confirmed? y/N', 'n').lower() != 'y':
                    continue
                while True:
                    entities = ask('BIDS entities as key=value (task/run/acq/dir/echo); s to defer',
