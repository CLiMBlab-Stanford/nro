...
from nro.firstlevels.contract import validate_completion
from nro.firstlevels.design import build_design as fit_design
from nro.firstlevels.estimation import meta_records
from nro.firstlevels.io import load_fit, save_fit
from nro.firstlevels.models import validate_model
from nro.firstlevels.task_models import load_task_model, register_model
from nro.firstlevels.compiler import compile_model, realize_run_node
from nro.firstlevels.module import run_module
from nro.firstlevels.statistics import RunFit, Estimate, aggregate, fit_glm, linear_combination


def load_model(identifier, config=None):
    """Compile the shipped task fixture for low-level estimator tests."""
    return compile_model(load_task_model(identifier), identifier,
...
def build_design(node, events, confounds, tr, config):
    ...
    return fit_design(realize_run_node(node, events, confounds, config), events, confounds, tr, config)
...
def test_general_contrast_commutes_with_common_run_aggregation(weights):
    ...
    expected_dof = expected_variance**2 / sum((w*w * fit.variance(c))**2 / fit.dof for w, fit in zip(weights, fits.values()))
...
def test_gls_preserves_unfiltered_observations_and_counts_retained_frames():
    ...
    retained[::11] = False
    times = np.arange(n)
    x = np.column_stack((np.ones(n), np.sin(2 * np.pi * .3 * times)))
...
def test_nested_summaries_keep_original_run_variance_contributions():
    ...
    direct = linear_combination(effects, [.25, .25, .5]).evaluate(fits)
    for key in direct:
...
def test_precision_weights_need_not_commute_and_ignore_zero_variance_locations():
    ...
    pooled = linear_combination([aggregate(a, fits, weighting="precision"),
                                 aggregate(b, fits, weighting="precision")], [1, -1]).evaluate(fits)
    contrast = aggregate([linear_combination([x, y], [1, -1]) for x, y in zip(a, b)],
                         fits, weighting="precision").evaluate(fits)
...
def test_task_coefficients_remain_nuisance_adjusted_with_rank_protection():
    from nro.firstlevels.models import event_design
    ...
    events = pd.DataFrame({"onset": [5, 35, 65, 95], "duration": [6]*4, "trial_type": ["S", "N"]*2})
    ...
    expected = np.linalg.lstsq(raw.to_numpy()[design.retained], data[design.retained], rcond=None)[0]
...
def test_gap_aware_ar1_null_calibration(estimated):
    ...
    retained[::13] = False
    times = np.arange(n)
    x = np.column_stack([np.sin(times * .23), rng.normal(size=n)])
    y = cholesky(.6**abs(times[:, None] - times[None, :]), lower=True) @ rng.normal(size=(n, 3000))
    grid = np.array([-.4, -.2, 0, .2, .4, .6, .8]) if estimated else np.array([.6])
    fit = fit_glm(y, x[retained], retained=retained, ar_grid=grid)
    values = Estimate({"r": np.array([1, 0])}).evaluate({"r": fit})
    rejection = np.mean(2*t.sf(abs(values["t"]), values["dof"]) < .05)
    ...
    assert .03 < rejection < (.08 if estimated else .07)
...
def test_ols_null_calibration():
    ...
    rejected = np.mean(2 * t.sf(np.abs(result["t"]), result["dof"]) < .05)
    assert .03 < rejected < .07
...
def test_statsmodels_cannot_enable_temporal_filtering(option):
    model = load_model("langlocSN/main")
    model["Nodes"][0]["Model"]["Options"] = {option: .1}
...
def test_filter_configuration_is_removed_and_rejected(option):
    ...
    with pytest.raises(ValueError, match="does not support temporal filtering"):
        fit_design(load_model("langlocSN/main")["Nodes"][0], pd.DataFrame(), pd.DataFrame(), 1, config)


def test_contract_declares_unfiltered_fit():
    from nro.firstlevels.contract import firstlevels_output_contract
...
def _synthetic_project(tmp_path, domain):
    ...
    for i in (1, 2):
        ...
        nib.save(nib.Nifti1Image(np.ones((2, 2, 2, 120), np.float32), np.eye(4)), raw / f"{stem}_bold.nii.gz")
        (raw / f"{stem}_bold.json").write_text('{"RepetitionTime": 1}')
        # Run 2 has S but no N.
        pd.DataFrame({"onset": [5, 30, 60, 90], "duration": [5]*4,
                      "trial_type": ["S", "N", "S", "N"] if i == 1 else ["S"]*4}).to_csv(raw / f"{stem}_events.tsv", sep="\t", index=False)
        pd.DataFrame({"csf": rng.normal(size=120), "white_matter": rng.normal(size=120),
                      "rot_x": rng.normal(size=120), "trans_x": rng.normal(size=120),
                      "motion_outlier00": [1] + [0]*119}).to_csv(functional / f"{stem}_desc-confounds_timeseries.tsv", sep="\t", index=False)
        if domain == "volume":
            path = functional / f"{stem}_space-T1w_desc-preproc_bold.nii.gz"
            nib.save(nib.Nifti1Image(rng.normal(size=(2, 2, 2, 120)).astype(np.float32) + 100, np.eye(4)), path)
            ...
            for hemi in ("L", "R"):
                ...
                nib.save(nib.GiftiImage(darrays=[nib.gifti.GiftiDataArray(rng.normal(size=8).astype(np.float32)) for _ in range(120)]), path)
...
def test_module_outputs_omissions_and_resumption(tmp_path, domain, smoothing):
    ...
    kwargs = dict(runs=discover_raw_runs(root / "sub-01"), participant="01", project_root=root,
                  preprocessing_id="main", config_id="main", model_id="langlocSN/main", model=model,
                  config=config, space="T1w" if domain == "volume" else "fsnative", smoothing=smoothing,
                  work_root=tmp_path / "work")
    path = run_module(**kwargs)
    assert validate_completion(path)[0]
    document = json.loads(path.read_text())
    assert any(o["contrast"] == "N" and o["reason"] == "missing_condition" for o in document["omissions"])
    assert not list(path.parent.glob("*run-02_contrast-N_*"))
    assert list(path.parents[2].glob("node-subject/sub-01/*contrast-SvN_stat-t_*.nii.gz")) if domain == "volume" else list(path.parents[2].glob("node-subject/sub-01/*contrast-SvN_hemi-L_stat-t_*.shape.gii"))
...
def test_all_censored_run_is_an_explicit_omission(tmp_path):
    ...
    config = ConfigStore().load_configuration("firstlevels", "main").values
    for path in root.glob("derivatives/preprocessing/main/sub-01/ses-01/func/*confounds_timeseries.tsv"):
        frame = pd.read_csv(path, sep="\t")
        frame["motion_outlier00"] = 1
        frame.to_csv(path, sep="\t", index=False)
    path = run_module(runs=discover_raw_runs(root / "sub-01"), participant="01", project_root=root,
                      preprocessing_id="main", config_id="main", model_id="langlocSN/main",
                      model=load_task_model("langlocSN/main"), config=config, space="T1w", smoothing=0,
                      work_root=tmp_path / "work")
    result = json.loads(path.read_text())
    assert validate_completion(path)[0]
    assert not result["records"]
    assert len(result["omissions"]) == 2
    assert all(record["reason"] == "unidentifiable_temporal_model" for record in result["omissions"])


def test_run_groupby_and_inherited_events(tmp_path):
    from nro.engine.bids import resolve_bids_table
    from nro.firstlevels.models import validate_run_groups
    root = _synthetic_project(tmp_path, "volume")
...
def test_functional_inputs_select_non_aroma_from_workflow(tmp_path):
    from nro.firstlevels.module import functional_paths
    root = _synthetic_project(tmp_path, "volume")
    run = discover_raw_runs(root / "sub-01")[0]
    assert "desc-preprocNoAROMA" in functional_paths(root, "main", run, "T1w", aroma_enabled=True)[0].name
    assert "desc-preproc_bold" in functional_paths(root, "main", run, "T1w", aroma_enabled=False)[0].name


def test_absent_named_dummy_contrast_is_recorded():
    from nro.firstlevels.estimation import run_records
    config = ConfigStore().load_configuration("firstlevels", "main").values
    node = load_model("langlocSN/main")["Nodes"][0]
...
def test_planner_targets_dependencies_runtime_and_purge_isolation(tmp_path):
    ...
    specs = build_subject_instances(project="demo", participant="01", module="firstlevels",
                                    workflow=workflow, registered=registered, registry=registry,
                                    bids_root=root.parent, spaces=("fsnative", "T1w"), smoothing_levels=(0, 2))
    ...
    assert {(s.entities["space"], s.entities["smoothing"]) for s in targets} == {("fsnative", "0"), ("fsnative", "2"), ("T1w", "0"), ("T1w", "2")}
    target = targets[0]
...
def test_bootstrap_discovers_existing_firstlevels_without_demand(tmp_path):
    ...
    from nro.orchestration.registry import Registry
    from nro.firstlevels.paths import artifact_root, instance_prefix
    root = _synthetic_project(tmp_path / "bids", "volume")
    anatomy = root / "sub-01/anat/sub-01_T1w.nii.gz"
    anatomy.parent.mkdir(parents=True)
    nib.save(nib.Nifti1Image(np.ones((2, 2, 2), np.float32), np.eye(4)), anatomy)
...
def task_store(tmp_path, monkeypatch):
    ...
    for task, variant, membership in (("langlocSN", "main", ["main"]),
...
def test_model_set_default_and_explicit_selection(task_store):
    from nro.firstlevels.task_models import select_models
...
def test_shared_model_selectors_preserve_task_variant_pairs(task_store):
    ...
    selection = core_selection(build_parser().parse_args([
        "-m", "firstlevels", "--task", "langlocSN", "--model-set", "experiment",
        "--run", "task=langlocSN,other", "-s", "T1w", "-S", "0"]))
    ...
    assert not matches_instance_selectors({**entities, "model": "main"}, selection.instance_entities)
...
def test_model_membership_does_not_change_contract_or_configuration(task_store, tmp_path):
    import yaml
    from nro.firstlevels.task_models import scientific_model, model_contract
    from nro.orchestration.planner import build_subject_instances
    from nro.orchestration.registry import Registry
    ...
    nib.save(nib.Nifti1Image(np.ones((2, 2, 2), np.float32), np.eye(4)), anatomy)
    workflow = ConfigStore().resolve("main")
    assert not {"models", "task", "model_documents"}.intersection(workflow.configuration("firstlevels").values)
    registry = Registry.for_project("demo", bids_root=root.parent)
    registered = registry.register_workflow(workflow)
    kwargs = dict(project="demo", participant="01", module="firstlevels", workflow=workflow,
                  registered=registered, registry=registry, bids_root=root.parent,
                  spaces=("T1w",), smoothing_levels=(0,), models=("main", "dev"))
    ...
    path.write_text(yaml.safe_dump({**model, "model_set": ["experiment"], "description": "New description"}))
...
def test_equivalent_model_edit_does_not_rerun_completed_module(tmp_path, edit):
    ...
    kwargs = dict(runs=discover_raw_runs(root / "sub-01"), participant="01", project_root=root,
                  preprocessing_id="main", config_id="main", model_id="langlocSN/main",
                  model=source, config=config, space="T1w", smoothing=0, work_root=tmp_path / "work")
    output = run_module(**kwargs)
    before = {p: Path(p).stat().st_mtime_ns for p in json.loads(output.read_text())["public_outputs"]}
    if edit == "membership":
        source["model_set"] = ["development"]
        source["description"] = "Edited without changing the analysis"
    elif edit == "qualified":
        source["contrasts"] = {name: {f"trial_type.{key}": value for key, value in weights.items()}
    ...
    compiled_path = next(Path(p) for p in before if p.endswith("_statsmodel.json") and "run-01" in p)
...
def test_event_transforms_and_hrf_are_source_aware():
    config = ConfigStore().load_configuration("firstlevels", "main").values
    source = {"predictors": ["trial_type.*", "modulator"], "hrf": "spm",
              "hrf_overrides": {"modulator": None},
              "transformations": [{"Name": "Factor", "Input": "trial_type"},
                                   {"Name": "Scale", "Input": "response", "Demean": True, "Rescale": False,
                                    "Groupby": ["trial_type"], "Output": "centered"},
                                   {"Name": "Product", "Input": ["trial_type.S", "centered"], "Output": "modulator"}],
              "contrasts": {"S": {"trial_type.S": 1}, "modulation": {"modulator": 1}}}
    events = pd.DataFrame({"onset": [5, 25, 45, 65], "duration": [5]*4,
                           "trial_type": ["S", "N", "S", "N"], "response": [1., 10., 3., 20.]})
    confounds = pd.DataFrame({"csf": np.arange(100, dtype=float), "global_signal": np.arange(100, dtype=float)})
...
@pytest.mark.parametrize("field", ["confounds_regex", "models", "task", "levels", "noise_model", "Nodes"])
...
def test_advanced_blocks_are_validated_and_cannot_select_confounds():
    config = ConfigStore().load_configuration("firstlevels", "main").values
    source = {"predictors": ["trial_type.*"], "statsmodels": {
        "Transformations": {"Transformer": "pybids-transforms-v1", "Instructions": [{"Name": "Factor", "Input": "trial_type"}]},
        "Contrasts": [{"Name": "S", "ConditionList": ["trial_type.S"], "Weights": [1], "Test": "t"}]}}
...
def test_exact_outliers_bypass_nuisance_rank_cap():
    ...
    confounds = pd.DataFrame({"csf": np.arange(80, dtype=float), "motion_outlier00": [1] + [0]*79})
    design = build_design(model["Nodes"][0], events, confounds, 1, config)
...
def test_compiled_contrasts_commute_when_all_conditions_share_runs():
    ...
    from nro.firstlevels.estimation import run_records, evaluate_recipe
    model = load_model("langlocSN/main")
    fits = _fits()
    records = []
    for key in fits:
        run, _ = run_records(model["Nodes"][0], ["intercept", "trial_type.S", "trial_type.N", "other"],
                             SimpleNamespace(is_estimable=lambda _: True), key, {"subject": "01"})
        records.extend(run)
    pooled, omitted = meta_records(model["Nodes"][-1], records, model["Edges"][-1], weighting="equal")
    assert not omitted
    result = evaluate_recipe(next(r["recipe"] for r in pooled if r["name"] == "SvN"), fits).evaluate(fits)
...
def test_disjoint_conditions_produce_subject_contrast_but_no_run_contrasts(tmp_path):
    ...
    model = load_task_model("langlocSN/main")
    model["contrasts"] = {"SvN": {"S": .5, "N": -.5}}
    output = run_module(runs=discover_raw_runs(root / "sub-01"), participant="01", project_root=root,
                        preprocessing_id="main", config_id="main", model_id="langlocSN/main",
                        model=model, config=config, space="T1w", smoothing=0, work_root=tmp_path / "work")
...
def test_public_adoption_checks_scientific_model_not_set(tmp_path):
    from nro.firstlevels.contract import firstlevels_output_contract
    from nro.firstlevels.task_models import scientific_model
    from nro.orchestration.manifests import _public_derivative_completion, _PublicDerivativeContractMismatch
    source = load_task_model("langlocSN/main")
    scientific = scientific_model(source)
    path = tmp_path / "test_manifest.json"
    path.write_text(json.dumps({"complete": True, "task_model": source,
                               "output_metadata_contract": firstlevels_output_contract(), "public_outputs": []}))
    row = {"module": "firstlevels", "output_root": str(tmp_path), "expected_outputs_json": json.dumps([str(path)]),
    ...
    source["model_set"] = ["development"]
    row["artifact_contract_json"] = json.dumps({"processing": {"task_model": scientific_model(source)}})
    assert _public_derivative_completion(row, None)[0] is not None
...
def test_reference_levels_and_constant_group_handling():
    from nro.firstlevels.transforms import event_variables
    events = pd.DataFrame({"trial_type": ["A", "B", "A"], "x": [2., 2., 2.]})
    transformed, _, _ = event_variables(events, {"Instructions": [
        {"Name": "Factor", "Input": "trial_type", "Constraint": "drop_one", "RefLevel": "A"},
        {"Name": "Demean", "Input": "x"}]})
    ...
    with pytest.raises(ValueError, match="explicit RefLevel"):
        event_variables(events, {"Instructions": [{"Name": "Factor", "Input": "trial_type", "Constraint": "drop_one"}]})
