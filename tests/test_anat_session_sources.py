...
from nro.anat import module as anat_module
from nro.anat.common import AnatImage


def test_gray_matter_aseg_labels_have_named_definitions() -> None:
    segmentations = anat_module._FREESURFER_GRAY_MATTER_SEGMENTATIONS
    labels = anat_module._aseg_label_ids(segmentations)

    assert len(segmentations) == len(set(segmentations))
    assert set(labels) == {
        3, 8, 10, 11, 12, 13, 17, 18, 26, 28,
        42, 47, 49, 50, 51, 52, 53, 54, 58, 60,
    }
    assert anat_module._FREESURFER_ASEG_LABELS["Left-Cerebral-Cortex"] == 3
    assert anat_module._FREESURFER_ASEG_LABELS["Right-VentralDC"] == 60
...
def test_session_source_brain_extraction_consumes_preprocessed_image(
    ...
    plans = anat_module._plan_session_anatomicals(
        images=(image,),
        project="project",
    ...
    assert plan.staged_preprocessed == (
        ...
        / "anat"
        / "session_level"
        / "sub-1_ses-1_T1w_desc-preproc_T1w.nii.gz"
    )
...
def test_brain_extraction_reads_source_and_owns_its_outputs(tmp_path: Path) -> None:
    ...
    step = anat_module._brain_extract_anat_copy(
        synthstrip,
        env={},
        source=source,
        dst=destination,
