...
from nro.configuration.definitions import create_store, ensure_store, validate_store, _publish
from nro.configuration.store import ConfigStore, DERIVATIVE_CLASSES
from nro.firstlevels.task_models import load_task_model, scientific_model, select_models


def test_create_has_only_generic_starters(tmp_path):
    root = create_store(tmp_path / 'store')
    counts = validate_store(root)
    assert counts == dict(configs=7, workflows=3, models=0, event_ids=0, event_tsvs=0, bidsify=1)
...
@pytest.mark.parametrize('relative,text,match', [
...
def test_relocation_preserves_compiled_identities(definitions_fixture, tmp_path):
    ...
    assert scientific_model(load_task_model('langlocSN/main', copied / 'models')) == scientific_model(
        load_task_model('langlocSN/main', definitions_fixture / 'models'))
    (copied / '.git').mkdir()
...
def test_shared_filesystem_publication(tmp_path, monkeypatch):
    ...
    root = create_store(tmp_path / 'store')
    assert validate_store(root)['configs'] == 7
...
def test_setup_creates_selected_store_and_preserves_edits(tmp_path, monkeypatch):
    ...
    args = ['--resources-only', '--maintain', '--non-interactive', '--offline', '--without-oslom', '--local']
    setup.main(args)
