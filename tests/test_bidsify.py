...
from nro.bidsify.config import load_config, identifier, bids_label
from nro.bidsify.store import IngestionStore
from nro.bidsify.publication import approval_snapshot, publish, inventory
from nro.bidsify.pipeline import run_stage, convert
from nro.bidsify.images import extract, secure_directory, sanitize_image
from nro.bidsify.review import validate_events, compatible, issues
...
def ingestion(tmp_path):
    ...
    row = store.create(server='cni', remote_session='abc123', project='demo',
                       participant='01', session='01', config=config)
...
def test_duplicate_requests_and_concurrent_edits(ingestion):
    registry, store, row = ingestion
    again = store.create(server='cni', remote_session='abc123', project='demo', participant='01', session='01', config=row['config'])
...
def pending_request(store, config, *, remote='pending', session='ex123'):
    return store.create(server='cni', remote_session=remote, project='demo',
...
def test_source_session_cannot_change_destination_project(ingestion, state, replace):
    ...
    with pytest.raises(ValueError, match='belongs to BIDS project'):
        store.create(server='cni', remote_session=row['remote_session'], project='different',
                     participant=None, session=None, config=row['config'], replace=replace)
...
def test_rebidsify_retains_resolved_destination_and_sites_are_distinct(ingestion):
    ...
    with registry.connection(write=True):
        row['state'] = 'published'
        store.write_locked(row)
    repeated = store.create(server='cni', remote_session=row['remote_session'], project='demo',
                            config=row['config'], replace=True)
    assert (repeated['participant'], repeated['session']) == ('01', '01')
    with pytest.raises(ValueError, match='different BIDS destination'):
        store.create(server='cni', remote_session=row['remote_session'], project='demo',
                     participant='changed', config=row['config'], replace=True)
    other = store.create(server='lucas', remote_session=row['remote_session'], project='other',
                         config=row['config'])
...
def test_competing_projects_cannot_claim_same_source(ingestion):
    ...
    def create(project):
        try:
            return store.create(server='cni', remote_session='race', project=project, config=row['config'])
        except ValueError:
            return None
...
def test_image_preparation_does_not_require_identity(ingestion, monkeypatch):
    from nro.bidsify import pipeline
    registry, store, original = ingestion
    row = pending_request(store, original['config'], session=None)
    row.update(stage='prepare', acquisitions=[dict(id='a', datatype='anat', suffix='T1w',
                                                  confirmed=True, entities={})])
    prepared = []
    def prepare(record, item, helpers, source):
        ...
        return {'_shape': [2, 2, 2]}
    monkeypatch.setattr(pipeline, 'prepare_image', prepare)
...
def test_pending_identity_review_resumes_without_preparing_again(ingestion, monkeypatch):
    ...
    registry, store, original = ingestion
    row = pending_request(store, original['config'])
    row.update(state='needs_input', stage='convert', acquisitions=[
...
def test_shared_limit_and_worker_supply(ingestion):
    registry, store, row = ingestion
    store.create(server='cni', remote_session='other', project='demo', participant='02', session='01', config=row['config'])
    assert len(registry.reserve_worker_submissions(request_id=None, resource_class='large', memory_gb=32)) == 2
    assert registry.reserve_worker_submissions(request_id=None, resource_class='large', memory_gb=32) == []
    worker(registry, 'a')
    worker(registry, 'b')
...
def test_worker_finishes_stage_without_derivative_manifest(ingestion, monkeypatch):
    ...
    class Launcher:
        def run(self, envelope, **kwargs):
            ...
            (store.root / f"{row['id']}.result").write_text(json.dumps({'state': 'needs_input', 'stage': 'prepare', 'issues': ['review']}))
            kwargs['heartbeat']()
...
def test_publication_requires_exact_approval(ingestion):
    ...
    staged = Path(row['config']['staging']) / row['id'] / 'bids/sub-01/ses-01/anat/sub-01_ses-01_T1w.nii.gz'
...
def test_explicit_reference_compatibility():
    metadata = {'PhaseEncodingDirection': 'j', 'TotalReadoutTime': .03, '_shape': [2,2,2], '_affine': np.eye(4).tolist()}
    a = {'metadata': metadata}
    b = {'metadata': {**metadata, 'PhaseEncodingDirection': 'j-'}}
    assert compatible(a, b, opposite=True)
    assert not compatible(a, b)
    b['metadata']['TotalReadoutTime'] = .1
...
def test_real_dcm2bids_organizes_sanitized_nifti(ingestion, monkeypatch, pending):
    ...
    row['stage'] = 'convert'
    item = {'id': 'a', 'datatype': 'func', 'suffix': 'bold', 'confirmed': True,
            'entities': {'task': 'rest', 'run': '01'}, 'events': None, 'sbref': None, 'fieldmaps': [],
            'metadata': {'RepetitionTime': 2, '_shape': [2,2,2,10]}}
    row['acquisitions'] = [item]
    helper = Path(row['config']['staging']) / row['id'] / 'helpers/a'
    helper.mkdir(parents=True)
    nib.save(nib.Nifti1Image(np.ones((2,2,2,10), np.float32), np.eye(4)), helper / 'image.nii.gz')
...
def test_raw_anatomy_never_reaches_shared_staging(ingestion, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(pydicom, 'dcmread', lambda *_args, **_kwargs: SimpleNamespace(Modality='MR'))
    item = {'id': 'a', 'acquisition': 'a', 'file_token': 'hash', 'bytes': 3, 'datatype': 'anat', 'suffix': 'T1w', 'source_revision': {}}
    def download(_, path):
    ...
    def execute(argv, **kwargs):
        if '-b' in argv:
            ...
            nib.save(nib.Nifti1Image(np.ones((2,2,2), np.float32), np.eye(4)), output / 'converted.nii.gz')
            (output / 'converted.json').write_text(json.dumps({'PatientName': 'private', 'SeriesNumber': 1}))
        else:
            assert Path(argv[argv.index('-i') + 1]).is_relative_to(raw_root)
            nib.save(nib.Nifti1Image(np.full((2,2,2), 2, np.float32), np.eye(4)), argv[argv.index('-o') + 1])
...
def test_flywheel_revision_pinned_and_labels_not_saved(tmp_path):
    from nro.bidsify.flywheel import FlywheelSource
    file = SimpleNamespace(name='private-person-name.dicom.zip', type='dicom', size=3, version=1,
                           hash='opaque', modified='2026-01-01', download=lambda p: Path(p).write_bytes(b'abc'))
    acquisition = SimpleNamespace(id='abc', label='T1w private label', files=[file])
    client = SimpleNamespace(get_session=lambda _: SimpleNamespace(acquisitions=SimpleNamespace(iter=lambda: iter([acquisition]))),
...
def test_wizard_multiple_event_files_resume(ingestion, monkeypatch, tmp_path):
    ...
    row.update(stage='prepare', state='needs_input', acquisitions=[
        {'id': f'a{i}', 'datatype': 'func', 'suffix': 'bold', 'entities': {}, 'events': None, 'confirmed': False}
        for i in (1, 2)])
    row = save_decision(store, row)
    monkeypatch.setattr(nro.bidsify.flywheel, 'FlywheelSource', lambda _: SimpleNamespace(describe=lambda a: a['id']))
    answers = iter(['', 'y', 'bad', 'task=language run=01', str(event_path), 'y',
                    '', 'y', 'task=language run=02', str(event_path), 'y'])
    monkeypatch.setattr('builtins.input', lambda _: next(answers))
    with store.review_session(row['id']) as token:
        result = wizard(store, row, review_token=token)
    assert result['state'] == 'queued' and result['stage'] == 'prepare'
    assert all(Path(a['events']).read_text() == event_path.read_text() for a in result['acquisitions'])
...
def test_pending_ingestion_allocations_can_be_stopped_by_owner(ingestion):
    import getpass
    registry, store, row = ingestion
    reservations = registry.reserve_worker_submissions(request_id=None, resource_class='large', memory_gb=32)
...
def test_review_leases_only_block_the_active_session(ingestion):
    ...
    second = store.create(server='cni', remote_session='second', project='demo', participant='02', session='01', config=first['config'])
    with store.review_session(first['id']):
        ...
        duplicate = store.create(server='cni', remote_session='abc123', project='demo', participant='01', session='01', config=first['config'])
...
def test_expired_review_cannot_write_renew_or_release_successor(ingestion, monkeypatch):
    ...
    with pytest.raises(ValueError, match='ownership changed'):
        store.update(row, expected_revision=row['revision'], review_token=old, event_files={'a': 'unaccepted'})
...
def test_event_updates_are_revision_checked_and_preserve_previous_snapshot(ingestion, monkeypatch):
    ...
    with store.review_session(row['id']) as token:
        saved = store.update(row, expected_revision=row['revision'], review_token=token, event_files={'a': 'accepted'})
        event_path = Path(saved['acquisitions'][0]['events'])
        assert event_path.read_text() == 'accepted'
        with pytest.raises(ValueError, match='concurrently'):
            store.update(row, expected_revision=row['revision'], review_token=token, event_files={'a': 'rejected'})
        ...
        with pytest.raises(OSError, match='record write failed'):
            store.update(saved, expected_revision=saved['revision'], review_token=token, event_files={'a': 'new snapshot'})
...
def test_interactive_exit_releases_only_current_session(ingestion, monkeypatch, answer):
    ...
    import nro.bidsify.flywheel
    registry, store, row = ingestion
    row.update(state='needs_input', stage='prepare', acquisitions=[
        {'id': 'a', 'datatype': 'anat', 'suffix': 'T1w', 'confirmed': False, 'entities': {}, 'events': None}])
    row = save_decision(store, row)
    monkeypatch.setattr(nro.bidsify.flywheel, 'FlywheelSource', lambda _: SimpleNamespace(describe=lambda _: 'test'))
    def prompt(_):
...
def test_occupied_review_is_skipped_without_starting_wizard(ingestion, monkeypatch, capsys):
    ...
    row['state'] = 'needs_input'
    row = save_decision(store, row)
    monkeypatch.setattr(cli, '_advance', lambda *_args, **_kwargs: pytest.fail('Occupied review started'))
    with store.review_session(row['id']) as token:
        cli.advance(store, row)
