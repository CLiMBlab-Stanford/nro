...
from nro.bidsify.discovery import existing_sessions, inferred_session, session_choices, validate_session_rules
from nro.bidsify.flywheel import FlywheelSource


def rules():
    return [
        dict(server='cni', remote_project='test/demo', match={'label': r'(?P<exam>[0-9]+)'},
             participant='ex{exam}', session=None),
        dict(server='cni', remote_project='test/demo', match={'label': r'(?P<exam>[0-9]+)'},
             participant=None, session='ex{exam}'),
    ]
...
def test_infer_session_only_from_agreeing_server_rules():
    ...
    assert inferred_session(remote(label='private display label'), server='cni', rules=rules()) is None
...
def test_cli_can_register_without_participant_and_infer_session(tmp_path, monkeypatch, capsys, configured):
    ...
    monkeypatch.setattr(cli, 'FlywheelSource', lambda _: SimpleNamespace(sessions=lambda: [remote()]))
    prompts = []
...
def discover(root, rows=None, **kwargs):
    return session_choices(rows if rows is not None else [remote()], bids_root=root, server='cni',
                           rules=kwargs.pop('rules', rules()), records=kwargs.pop('records', []), **kwargs)
...
@pytest.mark.parametrize('relative', [
    'demo/derivatives/anat/sub-ex123', 'demo/sourcedata/sub-ex123',
    'derivatives/sub-ex123', '.hidden/sub-ex123', '_ignored_bids_backups/sub-ex123',
    'demo/sub-ex123/ses-unrelated', 'demo/sub-ex123',
])
...
@pytest.mark.parametrize('state', ['queued', 'running', 'needs_input', 'failed', 'interrupted', 'awaiting_approval'])
...
@pytest.mark.parametrize('field,value', [
    ('server', 'unknown'), ('remote_project', 'wrong/project'), ('match', {}),
    ('match', {'label': '['}), ('match', {'label': 123}), ('match', {'unknown': '.*'}),
    ('participant', '{missing}'), ('participant', '{exam.__class__}'),
    ('participant', '../{exam}'), ('participant', '{exam!r}'),
])
...
def test_source_selection_requires_one_configured_project(monkeypatch):
    ...
    assert select_source(config, 'demo', server='cni', flywheel_project='lab/one') == ('cni', 'lab/one')
...
def test_multisite_mapping_filters_and_prompts_by_server(monkeypatch):
    ...
    assert select_source(config, 'demo', server='lucas', flywheel_project=None) == ('lucas', 'test/demo')
    answers = iter(['test/demo', 'lucas/test/demo'])
    monkeypatch.setattr('nro.bin.bidsify.ask', lambda *_: next(answers))
    assert select_source(config, 'demo', server=None, flywheel_project=None) == ('lucas', 'test/demo')
...
def test_rebidsify_cannot_redirect_external_session(tmp_path, monkeypatch):
    ...
    with pytest.raises(SystemExit) as error:
        cli.main(['--server', 'cni', '-P', 'different', '--rebidsify', '--bids-root', str(root), '--no-submit'])
...
@pytest.mark.parametrize('mapping', [None, [], {'demo': {}},
    {'demo': []},
...
def test_cli_only_contacts_selected_flywheel_project(tmp_path, monkeypatch, capsys, mapped):
    ...
    monkeypatch.setattr(cli, 'load_config', lambda _: config)
    monkeypatch.setattr(cli.Registry, 'for_project', lambda *a, **k: registry)
    monkeypatch.setattr(cli, 'FlywheelSource', lambda profile: FlywheelSource(profile, client=SimpleNamespace(lookup=lookup)))
    def answer(prompt, *args):
...
def test_cli_omits_existing_before_prompting_and_writes_no_records(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli, 'load_config', lambda _: profile)
    monkeypatch.setattr(cli, 'FlywheelSource', lambda _: SimpleNamespace(sessions=lambda: [remote()]))
    monkeypatch.setattr(cli, 'ask', lambda *a: pytest.fail('An existing external session was offered'))
    cli.main(['--server', 'cni', '-P', 'demo', '--bids-root', str(root), '--no-submit'])
