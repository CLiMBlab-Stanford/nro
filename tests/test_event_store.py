...
def test_complete_task_names_not_prefixes(tmp_path):
    ...
    assert [e.identifier for e in store.candidates("LANGLOC-SN")] == [
        "langlocSN/set1_run1",
        "langlocSN/set1_run2",
    ]
...
@pytest.mark.parametrize(
    "identifier", ["../x", "/langlocSN/main", "langlocSN/../main", "unknown/main"]
)
...
def test_catalog_and_explicit_rule_candidates(tmp_path):
    ...
    config = {
        "event_store": str(tmp_path),
        "event_rules": [{"task": "task", "pattern": str(other)}],
    }
