...
def definitions_fixture(tmp_path_factory):
    ...
    (task / 'index.yml').write_text(yaml.safe_dump({'tasks': ['example'], 'files': {
        variant: {'path': 'example/main.tsv', 'source_names': []} for variant in ('main', 'same')
    }}))
    profile = root / 'bidsify/main.yml'
    value = yaml.safe_load(profile.read_text())
    value['servers'] = {
        'cni': {'host': 'cni.example.org', 'credential_env': 'TEST_CNI_KEY', 'projects': ['test/demo']},
        'lucas': {'host': 'lucas.example.org', 'credential_env': 'TEST_LUCAS_KEY', 'projects': ['test/demo']},
    }
